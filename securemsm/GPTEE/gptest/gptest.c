...
/* UUID for gptest app */
TEEC_UUID destination = {0xcad10542,
                         0x34e4,
                         0x452d,
...
*********************************************************************/
static void print_text(char *intro_message, uint8_t const *text_addr,
                       unsigned int size) {
  unsigned int i;
...
int QTEEC_TEST_SharedMemoryAccessTest(int iterations) {
  ...
  operation.paramTypes = TEEC_PARAM_TYPES(
      TEEC_VALUE_INPUT, TEEC_MEMREF_PARTIAL_INOUT, TEEC_NONE, TEEC_NONE);
  ...
  if (result != TEEC_SUCCESS) goto cleanup7;
...
TEEC_Result QTEEC_TEST_TEEC_AllocateSharedMemory(int cancel_request) {
  ...
  if (cancel_request) TEEC_RequestCancellation(&operation);
...
TEEC_Result QTEEC_TEST_TEEC_RegisterSharedMemory(int cancel_request) {
  ...
  memset((void *)inputBuffer, 0xA, inputSize);
  memset((void *)outputBuffer, 0xD, outputSize);
...
TEEC_Result QTEEC_TEST_TEEC_TempMemory(int cancel_request) {
  ...
  /*[5] Issue command to copy date from buffer in param[0]
  to buffer in param[1]. */
  result = TEEC_InvokeCommand(&session, CMD_BASIC_COPY_BACK, &operation,
                              &returnOrigin);
  if (result != TEEC_SUCCESS) goto cleanup3;
...
static int (*test_func[])(void) = {
        [NOMINAL] = NULL, [ADVERSARIAL] = NULL, [REPEAT] = NULL,
        [STRESS] = NULL,
};
...
void internal_test_menu(void) {
  ...
  printf("    1 ------->  Test Initialize/deinitialize context (iterations)\n");
  printf("    2 ------->  Test open/close Session (iterations)\n");
  printf("    3 ------->  Test open multiple sessions/close all\n");
  printf("    4 ------->  Test Invoke command with AllocateSharedMemory API\n");
  printf(
      "    7 ------->  Test request cancellation with AllocateSharedMemory "
      "Type API\n");
  printf(
      "    100------>  Generic Test: Input cmd & value to send to TA (trusted "
      "APP)\n\n");
  ...
  printf(
      "    Input 2 (Applicable to test cases 1 - 10) iteration? --> enter "
      "number of iterations\n");
  printf(
      "    Input 3 (Applicable to test case 100 only) command ID? --> enter "
      "command ID to send to TA\n");
  printf(
      "    Input 4 (Applicable to test case 100 only) value in TEEC_operation? "
      "--> enter value to send to TA\n");
  printf(
      "   "
      "------------------------------------------------------------------------"
      "--------------------------\n\n");
...
int internal_test(int argc, char **argv) {
  ...
  if (verbose) printf("Running test case %d\n", test_number);
...
static unsigned int parse_command(int argc, char *const argv[]) {
  ...
  while ((command = getopt_long(argc, argv, "vih:e:", testopts, NULL)) != -1) {
    switch (command) {
      case 'v':
        verbose = 1;
        break;
