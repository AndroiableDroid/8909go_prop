...
UartController::UartController(BluetoothSocType soc_type)
  : soc_type_(soc_type), crash_dump_started_(false), soc_crashed(false),
...
{
  if (!crash_dump_started_) {
    return;
  }

  std::unique_lock<std::mutex> lk(cv_m);
  if(cv.wait_for(lk, std::chrono::seconds(10), [this]{ return !crash_dump_started_; })) {
