...
#define FLASH_BASE_CALDATA_OFFSET_SOC_1 0x22000    // Offset for writing swift caldata
...
#define BLOCK_SIZE_VAL 164    // Position of block size for the data

#define M_EEEPROM_BLOCK_READ_ID 0xC8

#define BLOCK_START_POS 200     // Position where block data starts

#define BD_READ_CMD_ID_POS 48

#define BD_READ_RESP_PARAM_POS 88

#define QC98XX_BD_READ_RESP_PARAM 0x7
       ...
       if (input_msg[TLV_PAYLOAD_PARAM_2] == 1) {
           setBoardDataCaptureFlag(1);
               ...
               if ((fd = open(FLASH_PARTITION, O_RDWR)) < 0) {
                   ...
                   if (deviceid == 0){
                   ...
                   } else if (deviceid == QC9888_DEVICE_ID || deviceid == QC9887_DEVICE_ID){
                      ...
                      DPRINTF(FTM_DBG_TRACE,
                        "Current radio device with id %d is not supported", deviceid);
                   }
...
ftm_wlan_rsp_pkt_type *win_host_handle_fw_resp (void *data, uint32_t data_len)
{
    ...
    if (deviceid == 0){
        ...
        if ((((uint8_t *)data)[BD_READ_CMD_ID_POS] == M_EEEPROM_BLOCK_READ_ID) &&
          (((uint8_t *)data)[BD_READ_RESP_PARAM_POS] == QC98XX_BD_READ_RESP_PARAM)){
            int dataSize;
            le_to_cpu32(((uint8_t *)data + BLOCK_SIZE_VAL), dataSize);
            DPRINTF(FTM_DBG_TRACE,
              "Capturing Caldata for QC98xx by FTM :: Size %d\n", dataSize);
            DPRINTF(FTM_DBG_TRACE,
              "BDbuffer Offset : %d \n",(resp_counter*QC98XX_BLOCK_SIZE));
            memcpy(BDbuffer + BDbuffer_offset,(void *)((uint8_t *)data + BLOCK_START_POS),
              dataSize);
            resp_counter = resp_counter + 1;
            BDbuffer_offset+=dataSize;
        }
    }
    return rsp;
}
