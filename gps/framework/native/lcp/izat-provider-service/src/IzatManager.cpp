...
int32 IzatManager::addRequest(const IzatRequest* request) {
    ...
    if (request) {
        ...
        LOC_LOGI(
                "LocTech-Value :: Provider: %d, Num Updates: %d, TTFF: %lld, "
                "Interval: %lld, Displacement: %f, Horizontal Accuracy: %d, "
                "Altitude Accuracy: %d, Bearing Accuracy: %d",
                request->getProvider (),
...
void IzatManager::printUlpLocation (UlpLocation * ulpLocation, GpsLocationExtended* locationExtended, enum loc_sess_status status) {
    if (ulpLocation) {
        ...
        (
            ...
            "Accuracy: %f, UTC Time: %lld, Position Source: %x, "
            "Location Session Status %d",
