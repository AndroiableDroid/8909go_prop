...
#define QC_LOC_FW_LOCK  "qc_loc_fw_lock"
...
void wakeLockAcquireOrRelease (bool lockRequest)
{
  ...
  do
  {
    if (lockRequest == true)
    {
      fd = open(WAKELOCK_FILE, O_WRONLY|O_APPEND);
    }
    else // unlock request
    {
      fd = open(WAKEUNLOCK_FILE, O_WRONLY|O_APPEND);
    }

    if (fd < 0)
    {
      log_error(TAG, "%s: open error: %d, %s", __FUNCTION__, errno, strerror (errno));
      break;
    }

    result = snprintf(wakelock_id_str, sizeof (wakelock_id_str),
                      "%s%llu", QC_LOC_FW_LOCK, wakelock_id);

    if ((result < 0) || (result >= (int)sizeof (wakelock_id_str)))
    {
      log_error(TAG, "%s: snprintf failed: %d", __FUNCTION__, result);
      break;
    }

    result = write(fd, wakelock_id_str, strlen(wakelock_id_str));

    if (result != (int) strlen (wakelock_id_str))
    {
      log_error(TAG, "%s: write error: %d, %s", __FUNCTION__, errno, strerror (errno));
      break;
    }
  } while (0);
