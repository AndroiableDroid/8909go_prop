...
)
{
    ...
    if ( uEnable && 0 != wReportRateHz && 0 != wBatchSampleCount )
    {
        if (m_uBatchMode)
        {
            ...
            Return<Result> ret = m_pzNDKProvider->m_pzEventQueue->enableSensor(
                    m_zSensor.sensorHandle, lSensorRateUs, lMaxBatchLatencyUs);
    ...
    {
        uEnable = false;
        Return<Result> ret = m_pzNDKProvider->m_pzEventQueue->disableSensor(m_zSensor.sensorHandle);
        if (!ret.isOk())
        {
...
(
    const ::android::hardware::sensors::V1_0::SensorInfo &zSensor
)
{
    if (m_lNdkSensorType == zSensor.type &&
            (!m_uBound || m_zSensor.minDelay > zSensor.minDelay))
    {
        m_uBound = true;
        m_zSensor = zSensor;
        m_uBatchMode = false;
...
{
    if (m_lNdkSensorType == e.sensorType)
    {
        float fSensorSamples[3] =
        {
            UNIT_CONVERT_ACCELERATION(e.u.vec3.x),
            UNIT_CONVERT_ACCELERATION(e.u.vec3.y),
            UNIT_CONVERT_ACCELERATION(e.u.vec3.z)
        };
...
)
{
    if (m_lNdkSensorType == e.sensorType)
    {
        float fSensorSamples[3] =
        {
            UNIT_CONVERT_GYRO(e.u.uncal.x),
            UNIT_CONVERT_GYRO(e.u.uncal.y),
            UNIT_CONVERT_GYRO(e.u.uncal.z)
        };
        processRawData(fSensorSamples, tEventTimeMs);
        return true;
    }
...
)
{
    if (m_lNdkSensorType == e.sensorType)
    {
        float fSensorSamples[3] =
        {
            UNIT_CONVERT_PRESSURE(e.u.scalar),
            0,
            0
        };
        processRawData(fSensorSamples, tEventTimeMs);
...
)
{
    if (m_lNdkSensorType == e.sensorType)
    {
        float fSensorSamples[3] =
        {
            UNIT_CONVERT_MAG(e.u.vec3.x),
            UNIT_CONVERT_MAG(e.u.vec3.y),
            UNIT_CONVERT_MAG(e.u.vec3.z)
        };
        processRawData(fSensorSamples, tEventTimeMs);
        return true;
    }
...
)
{
    if (m_lNdkSensorType == e.sensorType)
    {
        float fSensorSamples[3] =
        {
            UNIT_CONVERT_MAG(e.u.uncal.x),
            UNIT_CONVERT_MAG(e.u.uncal.y),
            UNIT_CONVERT_MAG(e.u.uncal.z)
        };
        processRawData(fSensorSamples, tEventTimeMs);
        return true;
    }
...
void NDKProvider::cleanupSensors()
{
    ...
    if (0 != m_pzEventQueue.get() && 0 != m_zLooper.get())
    {
        Return<bool> unlinked =
                m_pzEventQueue->unlinkToDeath((::android::hardware::hidl_death_recipient *)this);

        if (!unlinked.isOk())
        {
            SLIM_LOGE("Transaction error in unlinking from sensor service death:=%s",
                    unlinked.description().c_str());
        }
        else if (!unlinked)
        {
            SLIM_LOGW("Unable to link from sensor service death notifications");
        }
        else
        {
            SLIM_LOGD("Unlink from sensor service death notification successful");
        }
...
struct myCallback : IEventQueueCallback
{
    Return<void> onEvent(const Event &e)
    {
        ...
        */
        ...
        if (NDKProvider::eTIME_SOURCE_UNKNOWN == myPzNDKProvider->m_eTimeSource)
        {
            ...
            if (SensorType::META_DATA != e.sensorType &&
                rLastSampleTimestampNs < e.timestamp)
            {
                rLastSampleTimestampNs = e.timestamp;
            }
        ...
        int64_t tTimestampUs = nanoseconds_to_microseconds(e.timestamp) +
                rTimestampDeltaUs;
                ...
                ++it)
        {
            ...
            if (pzServiceState->m_uRunning &&
                    pzServiceState->processEvent(e, tTimestampMs))
            {
...
)
{
    ...
    do
    {
        ...
        manager = ISensorManager::getService();
        if ((sp<ISensorManager>)nullptr == manager)
        {
            SLIM_LOGE("ISensorManager manager is NULL.");
            break;
        }

        myPzNDKProvider = this;   /**< Owner */

        Return<void> ret = manager->createEventQueue(new myCallback(),
                [&queue, &res](const auto &q, auto r)
                {
                    queue = q; res = r;
                });

        if (!ret.isOk() || res != Result::OK)
        { /* error handling */
            SLIM_LOGE("Event queue is unavailable res=%s",
                    toString(res).c_str());
            break;
        }
        ...
        Return<bool> linked =
                queue->linkToDeath((::android::hardware::hidl_death_recipient *)this, 0);

        if (!linked.isOk())
        {
            SLIM_LOGE("Transaction error in linking to sensor service death:=%s",
                    linked.description().c_str());
            break;
        }
        else if (!linked)
        {
            SLIM_LOGW("Unable to link to sensor service death notifications");
            break;
        }
        else
        {
            SLIM_LOGD("Link to sensor service death notification successful");
        }

        m_pzEventQueue = queue;

        /* Get the available sensors */

        android::hardware::hidl_vec<::android::hardware::sensors::V1_0::SensorInfo> sensorHandleVec;

        Return<void> retSL = manager->getSensorList(
            [&sensorHandleVec, &res](const ::android::hardware::hidl_vec<::android::hardware::sensors::V1_0::SensorInfo>& list, Result result)
            {
        ...
        if (res != Result::OK)
        { /* error handling */
            SLIM_LOGE("getSensorList failed res=%s",
                    toString(res).c_str());
            break;
        }
        ...
        IF_SLIM_LOGV
        {
            for (ssize_t i = 0; i < sensorHandleVec.size(); i++)
            {
        ...
        for (StateVector::iterator it = m_zStateList.begin(); it != m_zStateList.end();)
        {
            ...
            if (pzState->isBound())
            {
                SLIM_LOGE(
                        ...
                        " Min Delay: %" PRId32 " us handle: %" PRId32,
                        pzState->m_zSensor.name.c_str(),
                        pzState->m_zSensor.vendor.c_str(),
                        pzState->m_zSensor.typeAsString.c_str(),
                        pzState->m_zSensor.type,
                        pzState->m_zSensor.minDelay,
                        pzState->m_zSensor.sensorHandle);
