...
extern "C" const FlpLocationInterface* get_flp_interface();
#endif
...
#else
        sFlpInterface = get_flp_interface();
#endif
...
void flp_batching_verify(int fStopOnSec)
{
    int ret;
    FlpBatchOptions options;
    memset(&options, 0, sizeof(FlpBatchOptions));
    options.flags |= FLP_BATCH_WAKEUP_ON_FIFO_FULL;
    options.period_ns = (int64_t)5*1000000000; // set period=5s

    sFlpInterface->start_batching(1, &options);
    pthread_mutex_lock(&gMutexLocation);
...
int main (int argc, char *argv[])
{
    ...
    if(gFlpBatchingVerify)
    {
        if(sFlpInterface)
        {
            ...
            sFlpInterface->stop_batching(1);
            sFlpInterface->cleanup();
        }
        else
        {
            printf("sFlpInterface is null, and cannot start flp batching\n");
            LOC_LOGE("sFlpInterface is null, and cannot start flp batching");
        }
        return 0;
    }
