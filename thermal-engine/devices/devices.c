...
int tmd_init_cpu_devs(void)
{
	...
	for (idx = 0; idx < num_cpus; idx += inc_idx) {
		int arr[50];
		uint32_t num_affected_cpus = 0;
		char path[DEVICES_MAX_PATH] = {0};
		...
		if (cluster_id != -1) {
			...
			snprintf(path, sizeof(path),
					CPUFREQ_POLICY(AFFECTED_CPUS), idx);
			num_affected_cpus = read_int_list_from_file(path,
						arr, ARRAY_SIZE(arr));
			if (num_affected_cpus > 0)
				inc_idx = num_affected_cpus;
		}
