...
static uint8_t cpr_band_supported;
...
int cpr_band_request(int request)
{
	...
	if (!cpr_band_supported) {
		msg("CPR temperature band sysfs not supported\n");
		return -1;
	}
	 ...
	 */
	snprintf(buf, UINT_BUF_MAX, "%d", cpr_lvl_band_table[request][1]);
	if ((write_to_file(CPR_BAND_SYSFS, buf, strlen(buf)) > 0) &&
	    (!cpr_band_qmi_request(request))) {
		thermalmsg(LOG_LVL_DBG,
			(LOG_LOGCAT | LOG_LOCAL_SOCKET | LOG_TRACE),
			"%s:CPR Temp band:%d\n", MITIGATION, request);
...
void cpr_band_init(void)
{
	...
	if (read_line_from_file(CPR_BAND_SYSFS, buf, sizeof(buf)) > 0) {
		cpr_band_supported = 1;
		info("%s: Init CPR temperature band supported: %d", __func__,
			atoi(buf));
		/*
		...
		info("%s: Could not read %s", __func__, CPR_BAND_SYSFS);
	}
}
#endif
