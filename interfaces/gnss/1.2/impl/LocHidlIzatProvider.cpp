...
public:
    ...
    inline void toggleEnabled() { mIsEnabled = !mIsEnabled; }
...
void LocHidlIzatProvider::cleanup() {
    if (mListener) {
        mListener->getHidlCbIface()->unlinkToDeath(mDeathRecipient);
        onDisable();
        delete mListener;
        mListener = NULL;
    }
...
Return<bool> LocHidlIzatProvider::onDisable() {
    ...
    if (mListener != NULL) {
        if (mListener->isEnabled()) {
            ...
            mIzatManager->unsubscribeListener(mListener);
            mListener->toggleEnabled();
        }
        success = true;
}
