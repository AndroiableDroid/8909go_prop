...
int ThreadHelper::createThread(Meter* mtr) {
    ...
    tname = getThreadName().substr(0,MAX_THREAD_NAME_LENGTH);
    retValue = pthread_setname_np(pthread_self(), tname.empty() ? "lm-unknown" : tname.c_str());
    if (retValue != 0) {
        DEBUGW(LOG_TAG_TH,"Failed to setname ThreadName: %s",getThreadName().c_str());
    }
