 ...
 */
DbRowVect FeatureState::createGetUniqueDR(string appName) {
    DbRowVect dbRow;
    if(!mStateDb) {
    ...
    if(dbRow.size() == 0) {
        /* If the app is not present in DB, Create it */
        DEBUGV(LOG_TAG_FS, "%s: App %s not present in db, creating", mFeatureName.c_str(), appName.c_str());
        createRow(key);
        /*
         * TODO: After this the dbRow with current app has size 1, but returned
         * dbRow has size 0. This would break when the writeState() is called before
         * the first readState()
         */
    }
    else if(dbRow.size() != 1) {
        /*
