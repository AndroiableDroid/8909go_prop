...
Target::Target() {
    ...
    mResolution = PerfBoostsStore::MAP_RES_TYPE_ANY;
    mTargetName = NULL;
    mTotalGpuFreq = 0;
    mTotalGpuBusFreq = 0;
}
...
int Target::readResolution() {
    int fd = -1;
    int ret = PerfBoostsStore::MAP_RES_TYPE_ANY, n = 0;
    int w = 0, h = 0, t = 0;
    char *pch = NULL;
    ...
    if (fd != -1) {
        ...
        if (n > 0) {
            ...
            if (w <= 800 && h <= 1280) {
                ret = PerfBoostsStore::MAP_RES_TYPE_720P;
            }
            else if (w <= 1080 && h <= 1920) {
                ret = PerfBoostsStore::MAP_RES_TYPE_1080P;
            }
            else if (w <= 1440 && h <= 2560) {
                ret = PerfBoostsStore::MAP_RES_TYPE_2560;
            }
        }
        close(fd);
    }
...
*/
int Target::getCpuMaxFreqResetVal(int cluster) {
...
int Target::getBoostConfig(int hintId, int type, int *mapArray, int *timeout) {
    ...
    if (NULL != mBoostConfigsStore) {
        size = mBoostConfigsStore->GetBoostConfig(hintId, type, mapArray, timeout,
