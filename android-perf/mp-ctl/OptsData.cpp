...
OptsData::OptsData() {
    ...
    cpubw_stored = 0;
    cpubw_ip_sl = -1;
    memlat_ratio_ceil_0_sl = -1;
    memlat_ratio_ceil_1_sl = -1;
    gpu_default_pwrlvl_sl = -1;
    gpu_min_pwrlvl_sl = -1;
    gpu_max_pwrlvl_sl = -1;
    gpu_min_freq_sl = -1;
    gpu_max_freq_sl = -1;
    gpu_bus_min_freq_sl = -1;
    gpu_bus_max_freq_sl = -1;
    lpm_ref_stddev_sl = -1;
    lpm_trm_add_sl = -1;
...
int OptsData::init_llcbw_hwmon_path() {
    ...
    snprintf(llcbw_minfreq_path, NODE_MAX, "%s/min_freq", llcbw_path);
...
int OptsData::init_l3_path() {
    ...
    /* for l3 little cluster */
    ret = init_devfreq_child_path("l3-cpu0", d_name);

    if (ret < 0)
        return FAILED;

    snprintf(l3l_path, NODE_MAX, "%s%s", DEVFREQ_PATH, d_name);
    snprintf(l3l_minfreq_path, NODE_MAX, "%s/min_freq", l3l_path);
    snprintf(l3l_maxfreq_path, NODE_MAX, "%s/max_freq", l3l_path);

    ret = init_devfreq_freqlist(l3l_path);

    if (ret == SUCCESS) {
        l3l_avail_freqs_n = avail_freqs_n;
        for (int i = 0; i < l3l_avail_freqs_n; i++) {
            l3l_avail_freqs[i] = avail_freqs[i];
            QLOGI("L3 CPU0, avail freq is %d", l3l_avail_freqs[i]);
        }
    }

    /* for l3 big cluster */
    ret = init_devfreq_child_path("l3-cpu4", d_name);

    if (ret < 0)
        return FAILED;

    snprintf(l3b_path, NODE_MAX, "%s%s", DEVFREQ_PATH, d_name);
    snprintf(l3b_minfreq_path, NODE_MAX, "%s/min_freq", l3b_path);
    snprintf(l3b_maxfreq_path, NODE_MAX, "%s/max_freq", l3b_path);

    ret = init_devfreq_freqlist(l3b_path);

    if (ret == SUCCESS) {
        l3b_avail_freqs_n = avail_freqs_n;
        for (int i = 0; i < l3b_avail_freqs_n; i++) {
            l3b_avail_freqs[i] = avail_freqs[i];
            QLOGI("L3 CPU4, avail freq is %d", l3b_avail_freqs[i]);
        }
        return SUCCESS;
    }
    else
