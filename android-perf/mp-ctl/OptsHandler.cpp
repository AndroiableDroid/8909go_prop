...
OptsHandler::OptsHandler():
    ...
    mOptsTable {
        ...
        /*Replace toggle_power_collapse func with pmqos API for same OpCode
         *ensuring Clients are abstracted from switch of API and prevent
         * client modifications.
         */
        /*Use PM QoS locks API for the same Opcode of toggle_PC*/
        {pmQoS_cpu_dma_latency, pmQoS_cpu_dma_latency, lower_is_better},
        /*Use L2 PC node for toggling*/
        {toggle_l2_pc_perf, toggle_l2_pc_perf, lower_is_better},
        {lpm_bias_hyst, lpm_bias_hyst, higher_is_better},
        {lpm_ref_stddev, lpm_ref_stddev, higher_is_better},
        {lpm_trm_add, lpm_trm_add, higher_is_better},
        ...
        /*sched group*/
        {sched_boost, sched_boost, lower_is_better},
        ...
        {sched_top_app_cpuset, sched_top_app_cpuset, always_apply},
        {sched_fg_cpuset, sched_fg_cpuset, always_apply},
        {sched_sys_bg_cpuset, sched_sys_bg_cpuset, always_apply},
        {sched_bg_cpuset, sched_bg_cpuset, always_apply},
        ...
        {sched_group_upmigrate, sched_group_upmigrate, lower_is_better},
        {sched_group_downmigrate, sched_group_downmigrate, lower_is_better},
        {sched_freq_aggr_threshold, sched_freq_aggr_threshold, lower_is_better},
        ...
        {gpu_min_freq, gpu_min_freq, higher_is_better},
        {gpu_max_freq, gpu_max_freq, higher_is_better},
        {gpu_bus_min_freq, gpu_bus_min_freq, higher_is_better},
        {gpu_bus_max_freq, gpu_bus_max_freq, higher_is_better},
        ...
        {disable_ppr, disable_ppr, higher_is_better},

        /*llcbw hwmon*/
        {llcbw_hwmon_min_freq, llcbw_hwmon_min_freq, higher_is_better},

        /*memlat*/
        {l3_min_freq, l3_min_freq, higher_is_better},
        {l2_memlat_ratio_ceil_0, l2_memlat_ratio_ceil_0, higher_is_better},
        {l2_memlat_ratio_ceil_1, l2_memlat_ratio_ceil_1, higher_is_better},
    }
{
}
...
int OptsHandler::pmQoS_cpu_dma_latency(Resource &r, OptsData &d) {
     ...
     */
     ...
     if (latency < min_pmqos_latency) {
         QLOGI("%s:Requested Latency=%x", __func__, latency);
         QLOGI("%s:Target Min WFI Latency is =%x, setting same", __func__,min_pmqos_latency);
         latency = min_pmqos_latency;
     } else {
         QLOGI("%s:Client latency=%x > trgt min latency=%x", __func__, latency,min_pmqos_latency);
...
/* Enable/disable L2 power collapse */
int OptsHandler::toggle_l2_pc_perf(Resource &r,  OptsData &d) {
    int rc = 0;
    char tmp_s[NODE_MAX];
    unsigned int reqval = r.value;
    static unsigned int stored_l2_state = 0;
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.toggle_l2_pc_sl > 0) {
            FWRITE_STR(TOGGLE_L2_PC_PERF, d.toggle_l2_pc_s, d.toggle_l2_pc_sl, rc);
            stored_l2_state = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", TOGGLE_L2_PC_PERF, d.toggle_l2_pc_s, rc);
        }
        return rc;
    }

    if (!stored_l2_state) {
        FREAD_STR(TOGGLE_L2_PC_PERF, d.toggle_l2_pc_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", TOGGLE_L2_PC_PERF, d.toggle_l2_pc_s, rc);
            d.toggle_l2_pc_sl = strlen(d.toggle_l2_pc_s);
        } else {
            QLOGE("Failed to read %s", TOGGLE_L2_PC_PERF);
            return FAILED;
        }
        stored_l2_state = 1;
    }
    reqval = !!reqval; /* 0 or 1 */
    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(TOGGLE_L2_PC_PERF, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: writing %s with %s and rc is %d ", TOGGLE_L2_PC_PERF, tmp_s, rc);
    if (rc < 0) {
        QLOGE("Acquiring the L2 power collapse perflock failed\n");
    }
    return rc;
}

int OptsHandler::lpm_bias_hyst(Resource &r,  OptsData &d) {
    unsigned int reqval = r.value;
    char tmp_s[NODE_MAX];
    int rc = FAILED;
    static unsigned int stored_bias_hyst;
    static char bias_storage[NODE_MAX]; // bias storage
    static int bias_storage_len; // bias string length
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (bias_storage_len > 0) {
            FWRITE_STR(LPM_BIAS_HYST, bias_storage, bias_storage_len, rc);
            stored_bias_hyst = 0;
            QLOGI("perf_lock_rel: writing %s with %s and rc is %d", LPM_BIAS_HYST, bias_storage, rc);
            if (rc < 0) {
                QLOGE("perf_lock_rel: failed for %s\n", LPM_BIAS_HYST);
            }
        }
        return rc;
    }
    ...
    if (!stored_bias_hyst) {
        FREAD_STR(LPM_BIAS_HYST, bias_storage, NODE_MAX, rc);
        if (rc > 0) {
            QLOGI("%s read with %s return value %d", LPM_BIAS_HYST, bias_storage, rc);
            bias_storage_len = strlen(bias_storage);
            stored_bias_hyst = 1;
        } else {
            QLOGE("Failed to read %s", LPM_BIAS_HYST);
            return FAILED;
        }
    }

    if (reqval <= 0 || reqval >= MAX_LPM_BIAS_HYST) {
        QLOGI("Requested value is %d out of limits, resetting to default bias of %d", reqval, DEFAULT_LPM_BIAS_HYST);
        reqval = DEFAULT_LPM_BIAS_HYST;
    }

    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(LPM_BIAS_HYST, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: writing %s with %s and rc is %d ", LPM_BIAS_HYST, tmp_s, rc);
    if (rc < 0) {
        QLOGE("Acquiring the lock for %s failed\n", LPM_BIAS_HYST);
    }
    return rc;
}

int OptsHandler::lpm_ref_stddev(Resource &r, OptsData &d) {
    int rc = 0;
    unsigned int reqval = r.value, fval = 0;
    static unsigned int stored_val = 0;
    char tmp_s[NODE_MAX];
    const char *node = LPM_LEVELS_REF_STDDEV;
    char *node_val_str = d.lpm_ref_stddev_s;
    int &node_val_sl = d.lpm_ref_stddev_sl;
    ...
    if (reqval == MAX_LVL) {
        if (node_val_sl > 0) {
            FWRITE_STR(node, node_val_str, node_val_sl, rc);
            stored_val = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d",
                  node, node_val_str, rc);
        }
        return rc;
    }

    if (!stored_val) {
        FREAD_STR(node, node_val_str, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", node, node_val_str, rc);
            node_val_sl = strlen(node_val_str);
        } else {
            QLOGE("Failed to read %s", node);
            return FAILED;
        }
        stored_val = 1;
    }
    ...
    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(node, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d",
          node, tmp_s, rc);
    return rc;
}

int OptsHandler::lpm_trm_add(Resource &r, OptsData &d) {
    int rc = 0;
    unsigned int reqval = r.value, fval = 0;
    static unsigned int stored_val = 0;
    char tmp_s[NODE_MAX];
    const char *node = LPM_LEVELS_TMR_ADD;
    char *node_val_str = d.lpm_trm_add_s;
    int &node_val_sl = d.lpm_trm_add_sl;
...
bool is_sched_boost_nonzero() {
    char tmp_s[NODE_MAX];
    int rc = FAILED;
    bool sched_boost_nonzero = false;

    FREAD_STR(SCHED_BOOST_NODE, tmp_s, NODE_MAX, rc);
...
int OptsHandler::sched_boost(Resource &r, OptsData &d) {
    ...
    /* write 0 to sched_boost node only if its nonzero */
        if (is_sched_boost_nonzero() == true) {
           /* Reset sched boost */
...
int OptsHandler::sched_update_restore_cpuset(Resource &r, const char *cpuset_node,
                                             char *input_cpuset_str, bool is_fg) {
    ...
    /* cpuset update section: cpuset_str passed as NULL for update*/
    if (input_cpuset_str == NULL) {
        char *gen_cpuset_str = cpuset_bitmask_to_str(r.value);
        if(gen_cpuset_str != NULL) {
            QLOGI("%s: BITMASK=%d, STRING=%s", cpuset_node, r.value, gen_cpuset_str);

            /* Only for foreground cpuset: first update foreground/boost/cpus
            before updating foreground/cpus with required bitmask */
            if (is_fg == true)
                FWRITE_STR(SCHED_FOREGROUND_BOOST, gen_cpuset_str, CPUSET_STR_SIZE, rc);

            FWRITE_STR(cpuset_node, gen_cpuset_str, CPUSET_STR_SIZE, rc);
            delete[] gen_cpuset_str;
        } else
              QLOGE("Failed to update cpuset bitmask for %s with %s", cpuset_node, gen_cpuset_str);
    }

    /* cpuset restore section: cpuset_str passed as non-null */
    else if (input_cpuset_str != NULL) {
        FWRITE_STR(cpuset_node, input_cpuset_str, CPUSET_STR_SIZE, rc);

        /* Only for foreground cpuset: restore foreground/boost/cpus
         after restoring foreground/cpus with all cpus bitmask */
        if (rc > 0) {
            if (is_fg == true)
                FWRITE_STR(SCHED_FOREGROUND_BOOST, input_cpuset_str, CPUSET_STR_SIZE, rc);
        } else
            QLOGE("Failed to restore cpuset bitmask for %s with %s", cpuset_node, input_cpuset_str);
    }
    return rc;
}

int OptsHandler::sched_top_app_cpuset(Resource &r, OptsData &d) {
    int rc = FAILED;
    static char cpuset_top_app_s[NODE_MAX];
    static unsigned int stored_top_app;
    static int cpuset_top_app_sl = -1;
    unsigned int reqval = r.value;

    if (reqval == MAX_LVL) {
        if (cpuset_top_app_sl > 0) {
            rc = sched_update_restore_cpuset(r, SCHED_TOP_APP_CPUSET, cpuset_top_app_s);
            stored_top_app = 0;
        }
        return rc;
    }
    ...
    if (!stored_top_app) {
        FREAD_STR(SCHED_TOP_APP_CPUSET, cpuset_top_app_s, NODE_MAX, rc);
        if (rc > 0) {
            cpuset_top_app_sl = strlen(cpuset_top_app_s);
            stored_top_app = 1;
        } else {
            QLOGE("Failed to update top app cpuset");
            return FAILED;
        }
    }

    rc = sched_update_restore_cpuset(r, SCHED_TOP_APP_CPUSET);
    return rc;
}

int OptsHandler::sched_fg_cpuset(Resource &r, OptsData &d) {
    int rc = FAILED;
    static char cpuset_fg_s[NODE_MAX];
    static unsigned int stored_fg;
    static int cpuset_fg_sl = -1;
    unsigned int reqval = r.value;

    if (reqval == MAX_LVL) {
        if (cpuset_fg_sl > 0) {
            rc = sched_update_restore_cpuset(r, SCHED_FOREGROUND_CPUSET, cpuset_fg_s, true);
            stored_fg = 0;
        }
        return rc;
    }
    ...
    if (!stored_fg) {
        /* Assumption: foreground/boost/cpus and foreground/cpus
         have same bitmask at all times */
        FREAD_STR(SCHED_FOREGROUND_CPUSET, cpuset_fg_s, NODE_MAX, rc);
        if (rc > 0) {
             cpuset_fg_sl = strlen(cpuset_fg_s);
            stored_fg = 1;
        } else {
            QLOGE("Failed to update fg cpuset");
            return FAILED;
        }
    }

    rc = sched_update_restore_cpuset(r, SCHED_FOREGROUND_CPUSET, NULL, true);
    return rc;
}

int OptsHandler::sched_sys_bg_cpuset(Resource &r, OptsData &d) {
    int rc = FAILED;
    static char cpuset_sys_bg_s[NODE_MAX];
    static unsigned int stored_sys_bg;
    static int cpuset_sys_bg_sl = -1;
    unsigned int reqval = r.value;

    if (reqval == MAX_LVL) {
        if (cpuset_sys_bg_sl > 0) {
            rc = sched_update_restore_cpuset(r, SCHED_SYSTEM_BACKGROUND_CPUSET, cpuset_sys_bg_s);
            stored_sys_bg = 0;
        }
        return rc;
    }
   if (!stored_sys_bg) {
        FREAD_STR(SCHED_SYSTEM_BACKGROUND_CPUSET, cpuset_sys_bg_s, NODE_MAX, rc);
        if (rc > 0) {
            cpuset_sys_bg_sl = strlen(cpuset_sys_bg_s);
            stored_sys_bg = 1;
        } else {
            QLOGE("Failed to update sys bg cpuset");
            return FAILED;
        }
    }

    rc = sched_update_restore_cpuset(r, SCHED_SYSTEM_BACKGROUND_CPUSET);
    return rc;
}

int OptsHandler::sched_bg_cpuset(Resource &r, OptsData &d) {
    int rc = FAILED;
    static char cpuset_bg_s[NODE_MAX];
    static unsigned int stored_bg;
    static int cpuset_bg_sl = -1;
    unsigned int reqval = r.value;

    if (reqval == MAX_LVL) {
        if (cpuset_bg_sl > 0) {
            rc = sched_update_restore_cpuset(r, SCHED_BACKGROUND_CPUSET, cpuset_bg_s);
            stored_bg = 0;
        }
        return rc;
    }
    ...
    if (!stored_bg) {
        FREAD_STR(SCHED_BACKGROUND_CPUSET, cpuset_bg_s, NODE_MAX, rc);
        if (rc > 0) {
            cpuset_bg_sl = strlen(cpuset_bg_s);
            stored_bg = 1;
        } else {
            QLOGE("Failed to update bg cpuset");
            return FAILED;
        }
    }

    rc = sched_update_restore_cpuset(r, SCHED_BACKGROUND_CPUSET);
    return rc;
}
...
/* Set sched_group_upmigrate */
int OptsHandler::sched_group_upmigrate(Resource &r,  OptsData &d) {
    int rc = 0;
    char tmp_s[NODE_MAX];
    static unsigned int stored_sched_group_upmigrate = 0;
    static char sch_grp_upmig_s[NODE_MAX];                // sched upmigrate storage
    static int  sch_grp_upmig_sl = -1;                    // sched upmigrate string length
    unsigned int reqval = r.value;
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (sch_grp_upmig_sl > 0) {
            FWRITE_STR(SCHED_GROUP_UPMIGRATE, sch_grp_upmig_s, sch_grp_upmig_sl, rc);
            stored_sched_group_upmigrate = 0;
            QLOGI("perf_lock_rel: updated %s with %s return value %d", SCHED_GROUP_UPMIGRATE, sch_grp_upmig_s, rc);
        }
        return rc;
    }

    if (!stored_sched_group_upmigrate) {
        FREAD_STR(SCHED_GROUP_UPMIGRATE, sch_grp_upmig_s, NODE_MAX, rc);
        if (rc > 0) {
            QLOGI("%s read with %s return value %d", SCHED_GROUP_UPMIGRATE, sch_grp_upmig_s, rc);
            sch_grp_upmig_sl = strlen(sch_grp_upmig_s);
            stored_sched_group_upmigrate = 1;
        }
        else {
            //better return fail, we should not acq lock when we can not release
            QLOGE("Failed to read %s", SCHED_GROUP_UPMIGRATE);
            return FAILED;
        }
    }

    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(SCHED_GROUP_UPMIGRATE, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s return value %d", SCHED_GROUP_UPMIGRATE, tmp_s, rc);
    return rc;
}

/* Set sched_group_downmigrate */
int OptsHandler::sched_group_downmigrate(Resource &r, OptsData &d) {
    int rc = 0;
    char tmp_s[NODE_MAX];
    static unsigned int stored_sched_group_downmigrate = 0;
    static char sch_grp_dwnmig_s[NODE_MAX];               // sched grp downmigrate storage
    static int  sch_grp_dwnmig_sl = -1;                   // sched grp downmigrate string length
    unsigned int reqval = r.value;
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (sch_grp_dwnmig_sl > 0) { //todo: verify: bug in original code
            FWRITE_STR(SCHED_GROUP_DOWNMIGRATE, sch_grp_dwnmig_s, sch_grp_dwnmig_sl, rc);
            stored_sched_group_downmigrate = 0;
            QLOGI("perf_lock_rel: updated %s with %s return value %d", SCHED_GROUP_DOWNMIGRATE, sch_grp_dwnmig_s, rc);
        }
        return rc;
    }

    if (!stored_sched_group_downmigrate) {
        FREAD_STR(SCHED_GROUP_DOWNMIGRATE, sch_grp_dwnmig_s, NODE_MAX, rc);
        if (rc > 0) {
            QLOGI("%s read with %s return value %d", SCHED_GROUP_DOWNMIGRATE, sch_grp_dwnmig_s, rc);
            sch_grp_dwnmig_sl = strlen(sch_grp_dwnmig_s);
            stored_sched_group_downmigrate = 1;
        } else {
            QLOGE("Failed to read %s", SCHED_GROUP_DOWNMIGRATE);
            return FAILED;
        }
    }

    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(SCHED_GROUP_DOWNMIGRATE, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s return value %d", SCHED_GROUP_DOWNMIGRATE, tmp_s, rc);
    return rc;
}

/* sched_freq_aggr_threshold perflock will have
 * contention with sched_boost level 3, so disable
 * this resource for the target which is going to
 * use sched boost 3 level.
 */
/* Set sched freq. aggregation threshold */
int OptsHandler::sched_freq_aggr_threshold(Resource &r, OptsData &d) {
    int rc = 0;
    char tmp_s[NODE_MAX];
    static unsigned int stored_sched_freq_aggr_threshold = 0;
    static char sch_freq_aggr_thres_s[NODE_MAX];               // sched freq aggr threshold storage
    static int  sch_freq_aggr_thres_sl = -1;                   // sched freq aggr threshold string length
    unsigned int reqval = r.value;
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (sch_freq_aggr_thres_sl > 0) {
            FWRITE_STR(SCHED_FREQ_AGGREGATE_THRESHOLD, sch_freq_aggr_thres_s, sch_freq_aggr_thres_sl, rc);
            stored_sched_freq_aggr_threshold = 0;
            QLOGI("perf_lock_rel: updated %s with %s return value %d", SCHED_FREQ_AGGREGATE_THRESHOLD, sch_freq_aggr_thres_s, rc);
        }
        return rc;
    }

    if (!stored_sched_freq_aggr_threshold) {
        FREAD_STR(SCHED_FREQ_AGGREGATE_THRESHOLD, sch_freq_aggr_thres_s, NODE_MAX, rc);
        if (rc > 0) {
            QLOGI("%s read with %s return value %d", SCHED_FREQ_AGGREGATE_THRESHOLD, sch_freq_aggr_thres_s, rc);
            sch_freq_aggr_thres_sl = strlen(sch_freq_aggr_thres_s);
            stored_sched_freq_aggr_threshold = 1;
        } else {
            QLOGE("Failed to read %s", SCHED_FREQ_AGGREGATE_THRESHOLD);
            return FAILED;
        }
    }

    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(SCHED_FREQ_AGGREGATE_THRESHOLD, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s return value %d", SCHED_FREQ_AGGREGATE_THRESHOLD, tmp_s, rc);
    return rc;
}
...
int OptsHandler::cpubw_hwmon_min_freq(Resource &r,  OptsData &d) {
    ...
    valid_bound = maxBoundCheck(hwmon_max_freq, d.cpubw_mf_s, tmp_s);
...
int OptsHandler::llcbw_hwmon_min_freq(Resource &r,  OptsData &d) {
    int rc = FAILED;
    char* setval = NULL;
    unsigned int reqval = r.value;
    static unsigned int stored_llcbw_hwmon_min_freq = 0;
    ...
    if (reqval == MAX_LVL) {
        if (d.llcbw_mf_sl > 0) {
            FWRITE_STR(d.llcbw_minfreq_path, d.llcbw_mf_s, d.llcbw_mf_sl, rc);
            QLOGI("perf_lock_rel: updated %s with %s return value %d", d.llcbw_minfreq_path, d.llcbw_mf_s, rc);
            stored_llcbw_hwmon_min_freq = 0;
        }
        return rc;
    }
    ...
    if (reqval > 0) {
        reqval *= 100;
        setval = get_devfreq_new_val(reqval, d.llcbw_avail_freqs, d.llcbw_avail_freqs_n, d.llcbw_maxfreq_path);
    }

    if (!stored_llcbw_hwmon_min_freq) {
        d.llcbw_mf_sl = save_devfreq_prev_val(d.llcbw_minfreq_path, d.llcbw_mf_s);
        if (d.llcbw_mf_sl > 0)
            stored_llcbw_hwmon_min_freq = 1;
    }

    if (setval != NULL) {
        FWRITE_STR(d.llcbw_minfreq_path, setval, strlen(setval), rc);
        QLOGI("perf_lock_acq: updated %s with %s return value %d", d.llcbw_minfreq_path, setval, rc);
        free(setval);
    }
...
int OptsHandler::l3_min_freq(Resource &r,  OptsData &d) {
    ...
    if (reqval == MAX_LVL) {
        if (d.l3l_mf_sl > 0 && d.l3b_mf_sl > 0) {
            FWRITE_STR(d.l3l_minfreq_path, d.l3l_mf_s, d.l3l_mf_sl, rc);
            QLOGI("perf_lock_rel_l3l: updated %s with %s return value %d", d.l3l_minfreq_path, d.l3l_mf_s, rc);
            FWRITE_STR(d.l3b_minfreq_path, d.l3b_mf_s, d.l3b_mf_sl, rc);
            QLOGI("perf_lock_rel_l3b: updated %s with %s return value %d", d.l3b_minfreq_path, d.l3b_mf_s, rc);
    ...
    if (reqval > 0) {
        reqval *= 100000;
        setval_l = get_devfreq_new_val(reqval, d.l3l_avail_freqs, d.l3l_avail_freqs_n, d.l3l_maxfreq_path);
        setval_b = get_devfreq_new_val(reqval, d.l3b_avail_freqs, d.l3b_avail_freqs_n, d.l3b_maxfreq_path);
    }

    if (!stored_l3_min_freq) {

        d.l3l_mf_sl = save_devfreq_prev_val(d.l3l_minfreq_path, d.l3l_mf_s);
        d.l3b_mf_sl = save_devfreq_prev_val(d.l3b_minfreq_path, d.l3b_mf_s);
        if (d.l3b_mf_sl > 0 && d.l3l_mf_sl > 0)
            stored_l3_min_freq = 1;
    }

    if (setval_l != NULL && setval_b != NULL) {
        FWRITE_STR(d.l3l_minfreq_path, setval_l, strlen(setval_l), rc);
        QLOGI("perf_lock_acq_l3l: updated %s with %s return value %d", d.l3l_minfreq_path, setval_l, rc);
        free(setval_l);
        FWRITE_STR(d.l3b_minfreq_path, setval_b, strlen(setval_b), rc);
        QLOGI("perf_lock_acq_l3b: updated %s with %s return value %d", d.l3b_minfreq_path, setval_b, rc);
        free(setval_b);
    }
...
int OptsHandler::cpubw_hwmon_max_freq(Resource &r,  OptsData &d) {
    ...
    valid_bound = minBoundCheck(d.cpubw_maxf_s, hwmon_min_freq, tmp_s);
...
int OptsHandler::gpu_min_freq(Resource &r, OptsData &d) {
    int rc = 0, fval = 0;
    unsigned int reqval = r.value;
    static unsigned int stored_gpu_min_freq = 0;
    char tmp_s[NODE_MAX];
    Target &target = Target::getCurTarget();
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.gpu_min_freq_sl > 0) {
            FWRITE_STR(GPU_MIN_FREQ, d.gpu_min_freq_s, d.gpu_min_freq_sl, rc);
            stored_gpu_min_freq = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", GPU_MIN_FREQ, d.gpu_min_freq_s, rc);
        }
        return rc;
    }

    if (!stored_gpu_min_freq) {
        FREAD_STR(GPU_MIN_FREQ, d.gpu_min_freq_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", GPU_MIN_FREQ, d.gpu_min_freq_s, rc);
            d.gpu_min_freq_sl = strlen(d.gpu_min_freq_s);
        } else {
            QLOGE("Failed to read %s", GPU_MIN_FREQ);
            return FAILED;
        }
        stored_gpu_min_freq = 1;
    }

    /* find the nearest >= available freq lvl, after multiplying
     * input value with 1000000 */
    reqval *= 1000000;
    fval = target.findNextGpuFreq(reqval);

    snprintf(tmp_s, NODE_MAX, "%d", fval);
    FWRITE_STR(GPU_MIN_FREQ, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d", GPU_MIN_FREQ, tmp_s, rc);
    return rc;
}


/* Set gpu max frequency*/
int OptsHandler::gpu_max_freq(Resource &r, OptsData &d) {
    int rc = 0;
    unsigned int reqval = r.value, fval = 0;
    static unsigned int stored_gpu_max_freq = 0;
    char tmp_s[NODE_MAX];
    Target &target = Target::getCurTarget();
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.gpu_max_freq_sl > 0) {
            FWRITE_STR(GPU_MAX_FREQ, d.gpu_max_freq_s, d.gpu_max_freq_sl, rc);
            stored_gpu_max_freq = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", GPU_MAX_FREQ, d.gpu_max_freq_s, rc);
        }
        return rc;
    }

    if (!stored_gpu_max_freq) {
        FREAD_STR(GPU_MAX_FREQ, d.gpu_max_freq_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", GPU_MAX_FREQ, d.gpu_max_freq_s, rc);
            d.gpu_max_freq_sl = strlen(d.gpu_max_freq_s);
        } else {
            QLOGE("Failed to read %s", GPU_MAX_FREQ);
            return FAILED;
        }
        stored_gpu_max_freq = 1;
    }
    ...
    snprintf(tmp_s, NODE_MAX, "%d", fval);
    FWRITE_STR(GPU_MAX_FREQ, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d", GPU_MAX_FREQ, tmp_s, rc);
    return rc;
}


/* Set gpu bus min frequency*/
int OptsHandler::gpu_bus_min_freq(Resource &r, OptsData &d) {
    int rc = 0, fval = 0;
    unsigned int reqval = r.value;
    static unsigned int stored_gpu_bus_min_freq = 0;
    char tmp_s[NODE_MAX];
    Target &target = Target::getCurTarget();
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.gpu_bus_min_freq_sl > 0) {
            FWRITE_STR(GPU_BUS_MIN_FREQ, d.gpu_bus_min_freq_s, d.gpu_bus_min_freq_sl, rc);
            stored_gpu_bus_min_freq = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", GPU_BUS_MIN_FREQ, d.gpu_bus_min_freq_s, rc);
        }
        return rc;
    }

    if (!stored_gpu_bus_min_freq) {
        FREAD_STR(GPU_BUS_MIN_FREQ, d.gpu_bus_min_freq_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", GPU_BUS_MIN_FREQ, d.gpu_bus_min_freq_s, rc);
            d.gpu_bus_min_freq_sl = strlen(d.gpu_bus_min_freq_s);
        } else {
            QLOGE("Failed to read %s", GPU_BUS_MIN_FREQ);
            return FAILED;
        }
        stored_gpu_bus_min_freq = 1;
    }

    /* find the nearest >= available freq lvl */
    fval = target.findNextGpuBusFreq(reqval);

    snprintf(tmp_s, NODE_MAX, "%d", fval);
    FWRITE_STR(GPU_BUS_MIN_FREQ, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d", GPU_BUS_MIN_FREQ, tmp_s, rc);
    return rc;
}


/* Set gpu bus max frequency*/
int OptsHandler::gpu_bus_max_freq(Resource &r, OptsData &d) {
    int rc = 0, fval = 0;
    unsigned int reqval = r.value;
    static unsigned int stored_gpu_bus_max_freq = 0;
    char tmp_s[NODE_MAX];
    Target &target = Target::getCurTarget();
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.gpu_bus_max_freq_sl > 0) {
            FWRITE_STR(GPU_BUS_MAX_FREQ, d.gpu_bus_max_freq_s, d.gpu_bus_max_freq_sl, rc);
            stored_gpu_bus_max_freq = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", GPU_BUS_MAX_FREQ, d.gpu_bus_max_freq_s, rc);
        }
        return rc;
    }

    if (!stored_gpu_bus_max_freq) {
        FREAD_STR(GPU_BUS_MAX_FREQ, d.gpu_bus_max_freq_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", GPU_BUS_MAX_FREQ, d.gpu_bus_max_freq_s, rc);
            d.gpu_bus_max_freq_sl = strlen(d.gpu_bus_max_freq_s);
        } else {
            QLOGE("Failed to read %s", GPU_BUS_MAX_FREQ);
            return FAILED;
        }
        stored_gpu_bus_max_freq = 1;
    }
    ...
    snprintf(tmp_s, NODE_MAX, "%d", fval);
    FWRITE_STR(GPU_BUS_MAX_FREQ, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d", GPU_BUS_MAX_FREQ, tmp_s, rc);
    return rc;
}
...
int OptsHandler::gpu_disable_gpu_nap(Resource &r, OptsData &d) {
    ...
    int ret[4] = {-1,-1,-1,-1};
    static char gpu_nap_s[NODE_MAX][4]; // input value node
...
int OptsHandler::disable_ppr(Resource &r, OptsData &d) {
    int rc = 0, fval = 0;
    unsigned int reqval = r.value;
    static unsigned int stored_disable_ppr_val = 0;
    char tmp_s[NODE_MAX];
    Target &target = Target::getCurTarget();
    ...
    rc = FAILED;
    if (reqval == MAX_LVL) {
        if (d.disable_ppr_sl > 0) {
            FWRITE_STR(DISABLE_PPR, d.disable_ppr_s, d.disable_ppr_sl, rc);
            stored_disable_ppr_val = 0;
            QLOGI("perf_lock_rel: updating %s with %s, return value %d", DISABLE_PPR, d.disable_ppr_s, rc);
        }
        return rc;
    }

    if (!stored_disable_ppr_val) {
        FREAD_STR(DISABLE_PPR, d.disable_ppr_s, NODE_MAX, rc);
        if (rc >= 0) {
            QLOGI("%s read with %s return value %d", DISABLE_PPR, d.disable_ppr_s, rc);
            d.disable_ppr_sl = strlen(d.disable_ppr_s);
        } else {
            QLOGE("Failed to read %s", DISABLE_PPR);
            return FAILED;
        }
        stored_disable_ppr_val = 1;
    }

    snprintf(tmp_s, NODE_MAX, "%d", reqval);
    FWRITE_STR(DISABLE_PPR, tmp_s, strlen(tmp_s), rc);
    QLOGI("perf_lock_acq: updated %s with %s, return value %d", DISABLE_PPR, tmp_s, rc);
    return rc;
}
...
int OptsHandler::l2_memlat_ratio_ceil_0(Resource &r, OptsData &d) {
    int rc = 0;
    unsigned int reqval = r.value, fval = 0;
    static unsigned int stored_val = 0;
    char tmp_s[NODE_MAX];
    const char *node = MEMLAT_RATIO_CEIL_0;
    char *node_val_str = d.memlat_ratio_ceil_0_s;
    int &node_val_sl = d.memlat_ratio_ceil_0_sl;
...
int OptsHandler::l2_memlat_ratio_ceil_1(Resource &r, OptsData &d) {
    int rc = 0;
    unsigned int reqval = r.value, fval = 0;
    static unsigned int stored_val = 0;
    char tmp_s[NODE_MAX];
    const char *node = MEMLAT_RATIO_CEIL_1;
    char *node_val_str = d.memlat_ratio_ceil_1_s;
    int &node_val_sl = d.memlat_ratio_ceil_1_sl;
