...
static void *mpctl_server(void *data) {
    ...
    if (rc != 0) {
        QLOGE("perflockERR: Unable to create work load detection polling thread\n");
        return NULL;
    }
...
static void *wrkld_detect_poll (void *data)
{
    ...
    QLOGI("perflock: Workload detect thread created\n");
            ...
            if (err < 0) {
                ...
                if(i == IO_PFD_IND) {
                    ...
                    if (io && !io_prof_handle) {
                        io_prof_handle = perf_lock_use_profile(io_prof_handle, IO_PROFILE);
                        QLOGE("perflock: IO ON %d\n", io);
                        ATRACE_INT("IO_mode", io);
                    } else if (!io && io_prof_handle) {
                        perf_lock_use_profile(io_prof_handle, -1);
                        io_prof_handle = 0;
