...
#define SCHED_GROUP_UPMIGRATE         "/proc/sys/kernel/sched_group_upmigrate"
#define SCHED_GROUP_DOWNMIGRATE       "/proc/sys/kernel/sched_group_downmigrate"
#define SCHED_FREQ_AGGREGATE_THRESHOLD       "/proc/sys/kernel/sched_freq_aggregate_threshold"
...
#define GPU_MIN_FREQ        "/sys/class/kgsl/kgsl-3d0/devfreq/min_freq"
#define GPU_MAX_FREQ        "/sys/class/kgsl/kgsl-3d0/devfreq/max_freq"
#define GPU_BUS_MIN_FREQ    "/sys/class/devfreq/soc:qcom,gpubw/min_freq"
#define GPU_BUS_MAX_FREQ    "/sys/class/devfreq/soc:qcom,gpubw/max_freq"
...
#define TOGGLE_L2_PC_PERF          "/sys/module/lpm_levels/system/perf/perf-l2-pc/idle_enabled"
#define LPM_BIAS_HYST           "/sys/module/lpm_levels/parameters/bias_hyst"
#define LPM_LEVELS_REF_STDDEV   "/sys/module/lpm_levels/parameters/ref_stddev"
#define LPM_LEVELS_TMR_ADD      "/sys/module/lpm_levels/parameters/tmr_add"
...
#define DISABLE_PPR                     "/sys/module/process_reclaim/parameters/enable_process_reclaim"

#define MEMLAT_RATIO_CEIL_0  "/sys/class/devfreq/soc:qcom,memlat-cpu0/mem_latency/ratio_ceil"
#define MEMLAT_RATIO_CEIL_1     "/sys/class/devfreq/soc:qcom,memlat-cpu4/mem_latency/ratio_ceil"
...
private:
    ...
    char llcbw_minfreq_path[NODE_MAX];
    char llcbw_maxfreq_path[NODE_MAX];
    unsigned int  llcbw_avail_freqs[MAX_FREQ_LLCBW];
    int  llcbw_avail_freqs_n;
    char llcbw_mf_s[NODE_MAX];             // cpubw min freq storage
    int  llcbw_mf_sl;                      // cpubw min freq string length
    char llcbw_maxf_s[NODE_MAX];           // cpubw max freq storage
    int  llcbw_maxf_sl;                    // cpubw max freq string length

    char l3l_path[NODE_MAX];
    char l3b_path[NODE_MAX];
    char l3l_minfreq_path[NODE_MAX];
    char l3b_minfreq_path[NODE_MAX];
    char l3l_maxfreq_path[NODE_MAX];
    char l3b_maxfreq_path[NODE_MAX];
    unsigned int  l3l_avail_freqs[MAX_FREQ_L3L];
    unsigned int  l3b_avail_freqs[MAX_FREQ_L3B];
    int  l3l_avail_freqs_n;
    int  l3b_avail_freqs_n;
    char l3l_mf_s[NODE_MAX];             // l3 little min freq storage
    int  l3l_mf_sl;                      // l3 little min freq string length
    char l3b_mf_s[NODE_MAX];             // l3 big min freq storage
    int  l3b_mf_sl;                      // l3 big min min freq string length
    ...
    char gpu_min_freq_s[NODE_MAX];        // gpu min freq storage
    int  gpu_min_freq_sl;                 // gpu min freq string length
    char gpu_max_freq_s[NODE_MAX];        // gpu max freq storage
    int  gpu_max_freq_sl;                 // gpu max freq string length
    char gpu_bus_min_freq_s[NODE_MAX];    // gpu bus min freq storage
    int  gpu_bus_min_freq_sl;             // gpu bus min freq string length
    char gpu_bus_max_freq_s[NODE_MAX];    // gpu bus max freq storage
    int  gpu_bus_max_freq_sl;             // gpu bus max freq string length

    char memlat_ratio_ceil_0_s[NODE_MAX];
    int memlat_ratio_ceil_0_sl;
    char memlat_ratio_ceil_1_s[NODE_MAX];
    int memlat_ratio_ceil_1_sl;

    char lpm_ref_stddev_s[NODE_MAX];
    int lpm_ref_stddev_sl;
    char lpm_trm_add_s[NODE_MAX];
    int lpm_trm_add_sl;
