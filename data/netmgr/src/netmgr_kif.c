...
LOCAL inline int
netmgr_kif_disable_pmtu
(
  boolean value
)
{
  int rc;
  char proc_file[NETMGR_KIF_MAX_COMMAND_LENGTH] = "/proc/sys/net/ipv4/ip_no_pmtu_disc";
  netmgr_cmdq_cfg_val_t val;

  if(value) {
    netmgr_log_high("%s DBG Disabling PMTU ip_no_pmtu_disc=%d", __func__, value);
  } else  {
    netmgr_log_high("%s DBG Enabling PMTU ip_no_pmtu_disc=%d", __func__, value);
  }
  ...
  if (NETMGR_SUCCESS != rc)
  {
    netmgr_log_err("%s(): failed to set PMTU value [%d]", __func__, value);
  }
  return rc;
}
  ...
  if ((link = netmgr_kif_get_link_for_ifindex((uint32)ifinfo->ifi_index)) < 0 ) {
    ...
    ** care about. */
    ...
    netmgr_log_low(" %s(): is_gro_update_req %d clat_iface_st %d"
		    ...
		    "bttether_iface_st:%d\n", __func__, is_gro_update_req,
		    clat_iface_st, softap_iface_st, rndis_iface_st,
