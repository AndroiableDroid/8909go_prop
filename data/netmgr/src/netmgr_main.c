...
netmgr_main (int argc, char ** argv)
{
  ...
  {
    log_flags = fcntl(fileno(logging_fd), F_GETFD);
    if (log_flags == -1)
    {
      perror("failed fcntl get");
    }
    else
    {
      log_flags |= FD_CLOEXEC;
      if (fcntl(fileno(logging_fd), F_SETFD, log_flags) == -1)
      {
        perror("failed fcntl set");
      }
    }
  }
#endif
...
#if defined (FEATURE_HANDLE_NETD_RESTART)
  NetmgrNetdClientInit();
#endif /* defined(FEATURE_HANDLE_NETD_RESTART) */
