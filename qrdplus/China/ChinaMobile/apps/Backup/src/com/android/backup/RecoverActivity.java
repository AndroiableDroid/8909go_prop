...
public class RecoverActivity extends ListActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        try {
            ...
            try {
                int mode = appOpsWrapper.checkOp(AppOpsManagerWrapper.OP_WRITE_SMS, info.applicationInfo.uid,
                        packageName);
                Log.v(TAG, packageName + " mode: " + mode);
                if (mode != AppOpsManager.MODE_ALLOWED) {
                    Log.v(TAG, packageName + " lost OP_WRITE_SMS:  (fixing)");
                    appOpsWrapper.setMode(AppOpsManagerWrapper.OP_WRITE_SMS, info.applicationInfo.uid,
