...
public class IrisManager {
    ...
    private static final int MSG_ENROLL_STATUS = 101;
    private static final int MSG_ENROLL_RESULT = 102;
    private static final int MSG_AUTHENTICATION_PROGRESS = 103;
    private static final int MSG_AUTHENTICATION_SUCCEEDED = 104;
    private static final int MSG_AUTHENTICATION_FAILED = 105;
    private static final int MSG_REMOVED = 106;
    ...
    public static final int IRIS_ERROR_PERMISSION_DENIED = 11;
    public static final int IRIS_ERROR_EYE_TOO_CLOSE = 12;
   ...
   /**
    ...
    public static final int IRIS_HELP_OUTDOOR = 15;
    public static final int IRIS_HELP_LIVENESS_FAILED = 16;
    public static final int IRIS_HELP_BLINK = 17;
    public static final int IRIS_HELP_MAX = 18;

    public static final int IRIS_CONFIGURE_ENROLL = 0x100;
    public static final int IRIS_CONFIGURE_AUTH = 0x200;

    public static final int IRIS_PREVIEW_SIZE_ENROLL = 0;
    public static final int IRIS_PREVIEW_SIZE_AUTH = 1;
    ...
    public void setPreviewSurface(Surface surface) {
        if (mService != null) {
            try {
                mService.setPreviewSurface(surface);
            } catch (RemoteException e) {
     ...
     */
    @RequiresPermission(USE_IRIS)
    public void configure(int id, byte[] param) {
        if (mService != null) {
            try {
                mService.configure(id, param);
            } catch (RemoteException e) {
                Log.v(TAG, "Remote exception in configure(): ", e);
            }
        } else {
            Log.w(TAG, "configure(): Service not connected!");
        }
    }
     ...
     */
    @RequiresPermission(USE_IRIS)
    public Size getPreviewSize(int use_case) {
        Rect previewRect;
        if (mService != null) {
            try {
                previewRect = mService.getPreviewSize(use_case);
                if (previewRect != null)
                    return new Size(previewRect.width(), previewRect.height());
                else {
                    Log.v(TAG, "fail to get preview size from IrisService");
                    return null;
                }
            } catch (RemoteException e) {
                Log.v(TAG, "Remote exception in configure(): ", e);
    ...
    private class MyHandler extends Handler {
        ...
        public void handleMessage(android.os.Message msg) {
            switch(msg.what) {
                case MSG_ERROR:
                    ...
                    break;
                case MSG_ENROLL_STATUS:
                    sendEnrollStatus((IrisOperationStatus)msg.obj);
                    break;                
                case MSG_ENROLL_RESULT:
                    sendEnrollResult();
                    break;
                case MSG_AUTHENTICATION_PROGRESS:
                    sendAuthenticateProgress((IrisOperationStatus)msg.obj);
                    break;
        ...
        private void sendEnrollStatus(IrisOperationStatus status) {
            if (mEnrollmentCallback != null) {
                Slog.v(TAG, "IrisManager enroll progress " + status.mProgress+" quality " + status.mQuality);
                if (status.mQuality > IRIS_HELP_NONE && status.mQuality < IRIS_HELP_MAX)
                    status.mHelpString = getHelpString(status.mQuality);

                 mEnrollmentCallback.onEnrollmentStatus(status);
            }
        }

        private void sendEnrollResult() {
            if (mEnrollmentCallback != null) {
                Slog.v(TAG, "IrisManager sendEnrollResult");
                mEnrollmentCallback.onEnrollmentDone();
            }
        }

        private void sendAuthenticateProgress(IrisOperationStatus status) {
            if (mAuthenticationCallback != null) {
                 if (status.mQuality > IRIS_HELP_NONE && status.mQuality < IRIS_HELP_MAX)
                    status.mHelpString = getHelpString(status.mQuality);

                 mAuthenticationCallback.onAuthenticationStatus(status);
            }
        }
    ...
    private String getErrorString(int errMsg) {
        switch (errMsg) {
            ...
            case IRIS_ERROR_PERMISSION_DENIED:
                return mContext.getString(
                    com.android.internal.R.string.iris_error_permission_denied);
            case IRIS_ERROR_EYE_TOO_CLOSE:
                return mContext.getString(
                         com.android.internal.R.string.iris_error_eye_too_close);
    ...
    private String getHelpString(int errMsg) {
         switch (errMsg) {
            ...
            case IRIS_HELP_OUTDOOR:
                return mContext.getString(
                    com.android.internal.R.string.iris_help_outdoor);
            case IRIS_HELP_LIVENESS_FAILED:
                return mContext.getString(
                    com.android.internal.R.string.iris_help_liveness_failed);
            case IRIS_HELP_BLINK:
                return mContext.getString(
                    com.android.internal.R.string.iris_help_blink);
    ...
    private IIrisServiceReceiver mServiceReceiver = new IIrisServiceReceiver.Stub() {
        ...
        @Override // binder call
        public void onEnrollStatus(long deviceId, IrisOperationStatus status) {
            Slog.v(TAG, "onEnrollStatus");
            mHandler.obtainMessage(MSG_ENROLL_STATUS, status).sendToTarget();
        }

        @Override // binder call
        public void onEnrollResult(long deviceId, int irisId, int groupId) {
            Slog.v(TAG, "onEnrollResult");
            mHandler.obtainMessage(MSG_ENROLL_RESULT).sendToTarget();
        }

        @Override // binder call
        public void onAuthenticationProgress(long deviceId, IrisOperationStatus status) {
            mHandler.obtainMessage(MSG_AUTHENTICATION_PROGRESS, status).sendToTarget();
        }
