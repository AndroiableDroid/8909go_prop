...
status_t IrisDaemonTestCallback::onEnrollResult(int64_t devId, int32_t irisId, int32_t groupId)
{
	printf("onEnrollResult 0x%llx, irisId=%d groupId=%d\n", devId, irisId, groupId);
	request_done = 1;
	return 0;
}

status_t IrisDaemonTestCallback::onEnrollStatus(int64_t devId, IrisOperationStatus& status)
{
	printf("onEnrollStatus 0x%llx, progress=%d\n", devId, status.progress);
	return 0;
}
...
status_t IrisDaemonTestCallback::onAuthStatus(int64_t devId, IrisOperationStatus& status)
{
	return 0;
}

status_t IrisDaemonTestCallback::onAuthResult(int64_t devId, bool matched, int32_t irisId, int32_t groupId)
{
	printf("onAuthResult 0x%llx, irisId=%d groupId=%d\n", devId, irisId, groupId);
	request_done = 1;
	return 0;
}
