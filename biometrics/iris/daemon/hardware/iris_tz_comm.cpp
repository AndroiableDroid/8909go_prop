...
int iris_tz_communicator::send_modified_cmd(void const *req_buf, size_t req_buf_len, void *rsp_buf, size_t rsp_buf_len, iris_ion_fd_info *fd_info)
{
	...
	shared_buf_req = qseecom_handle->ion_sbuffer;
	shared_buf_rsp = qseecom_handle->ion_sbuffer;
	shared_buf_rsp += IRIS_MAX_TZ_BUFFER_LEN >> 2;
	...
	for (i = 0; i < fd_info->fd_num; i++) {
		ion_fd_info.data[i].fd = fd_info->fd[i];
		ion_fd_info.data[i].cmd_buf_offset = fd_info->cmd_buf_offset[i];
		ALOGD("Send modified command: fd %d offset 0x%x,", fd_info->fd[i], fd_info->cmd_buf_offset[i]);
	}
	ALOGD("req_buf_len %d rsp_buf_len %d", req_buf_len, rsp_buf_len);
