...
//#define IRIS_CAMERA_DEBUG
#ifdef IRIS_CAMERA_DEBUG
#define IRIS_CAMERA_LOGD(fmt, args...) ALOGE("%s:%d"#fmt, __func__, __LINE__, ##args)
#else
#define IRIS_CAMERA_LOGD(fmt, args...) do{}while(0)
#endif
#define IRIS_CAMERA_LOGE(fmt, args...) ALOGE("%s:%d"#fmt, __func__, __LINE__, ##args)


#define PREVIEW_BUF_NUM 5   //minimum 5 preview buffer is required  1(tz)/1(backup)/2(camera)/1(free)
...
#define DEFAULT_SENSOR_EXP_TIME_MS  10
...
typedef struct {
    ...
    /* exposure time */
    uint64_t min_exp_time_ns;
    uint64_t max_exp_time_ns;
...
typedef struct {
    ...
    int registered_buf_fd[CAM_MAX_NUM_BUFS_PER_STREAM];
    size_t registered_buf_size;
    int registered_buf_num;
    iris_buf_state_t registered_buf_state[CAM_MAX_NUM_BUFS_PER_STREAM];
...
int iris_camera_get_caps(iris_camera_handle *handle,
...
int iris_camera_open_preview(iris_camera_handle *handle, bool is_rdi, bool is_secure);
...
int iris_camera_register_preview_buf(iris_camera_handle *handle, int buf_fd,
