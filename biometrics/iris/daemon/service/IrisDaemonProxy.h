...
public:
	...
	virtual int32_t setPreviewSurface(const sp<IGraphicBufferProducer>& bufferProducer);
	virtual int32_t configure(int32_t param_id, const uint8_t *param, int32_t param_size);
	virtual int32_t getPreviewSize(int use_case, int32_t *width, int32_t *height);
...
private:
	...
	static void copy_enroll_status(IIrisDaemonCallback::IrisOperationStatus& status, const iris_enroll_t& enroll_status);
	static void copy_auth_status(IIrisDaemonCallback::IrisOperationStatus& status, const iris_authenticate_t& auth_status);
	static void copy_frame_desc(IIrisDaemonCallback::IrisFrameDesc& desc ,const struct iris_frame_desc *frame_desc);
