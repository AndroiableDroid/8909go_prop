...
enum {
	CONFIGURE_PARAM_ENROLL = 0x100,
	CONFIGURE_PARAM_AUTH = 0x200,
	CONFIGURE_PARAM_ORIENTATION = 0x300,
};
...
public:
	...
	enum {
		...
		SET_PREVIEW_SURFACE = IBinder::FIRST_CALL_TRANSACTION + 13,
		CONFIGURE = IBinder::FIRST_CALL_TRANSACTION + 14,
		GET_PREVIEW_SIZE = IBinder::FIRST_CALL_TRANSACTION + 15,
	};
	...
	virtual int32_t setPreviewSurface(const sp<IGraphicBufferProducer>& bufferProducer) = 0;
	virtual int32_t configure(int32_t param_id, const uint8_t *param, int32_t param_size) = 0;
	virtual int32_t getPreviewSize(int use_case, int32_t *width, int32_t *height) = 0;
};
