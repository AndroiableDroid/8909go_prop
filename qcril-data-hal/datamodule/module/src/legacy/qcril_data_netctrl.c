...
bail:
  ...
  if ((QMI_NO_ERR !=rc) && (NULL != dsd_ind_sys_status_msg)) free(dsd_ind_sys_status_msg);

  // These should be freed as we are sending them "on stack"
  if (NULL != dsd_ind_curr_dds_msg) free(dsd_ind_curr_dds_msg);
  if (NULL != dsd_ind_switch_dds_msg) free(dsd_ind_switch_dds_msg);
