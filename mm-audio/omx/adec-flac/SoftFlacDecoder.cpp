...
SoftFlacDecoder::SoftFlacDecoder(
    ...
    : SimpleSoftOMXComponent(name, callbacks, appData, component),
      ...
      mLastAnchorTimeUs(-1),
      mUnDecodedBufferAnchorTimeUs(-1) {
    ...
    while (!inQueue.empty() && !outQueue.empty()) {
        ...
        if (mNotifyInputBufferEmpty) {
            ...
            if (inHeader->nFlags & OMX_BUFFERFLAG_EOS) {
                ...
                if (mAnchorTimeUs == -1)
                    mUnDecodedBufferAnchorTimeUs = inHeader->nTimeStamp;
        ...
        if (checkIfEnoughDecodedPCMSamples(mDecodedBlocks) || mBuffer.eos || skipBuffering) {
            ...
            outHeader->nOffset = 0;
            //If current input buffer timestamp is same as last input buffer timestamp then
            //treat this is a byte stream mode. Timestamp is completely based on decoded
            //frames so for
            if (mAnchorTimeUs == mLastAnchorTimeUs) {
                outHeader->nTimeStamp = mAnchorTimeUs +
                    (mNumFramesOutput * 1000000ll) / mSamplingRate;
            } else {
                //Input buffer timestamp is valid in this case, output timestamp is calculated
                //based on Input buffer timestamp and decoded frames from the current input
                //buffer
                numSamples = outHeader->nFilledLen /(mNumChannels * mOutBytesPerSample);
                outHeader->nTimeStamp =  mUnDecodedBufferAnchorTimeUs + ((numSamples * 1000000ll)/mSamplingRate);
                mUnDecodedBufferAnchorTimeUs = mAnchorTimeUs;
            }
        ...
        mLastAnchorTimeUs = mAnchorTimeUs;
    }
}
