...
void qap_session_callback(qap_session_handle_t session_handle __unused, void* priv_data __unused, qap_callback_event_t event_id, int size __unused, void *data)
{
    ...
    switch (event_id) {
        ...
        case QAP_CALLBACK_EVENT_EOS_ASSOC:
            ALOGD("%s %d Associated EOS Received", __func__, __LINE__);
        ...
        case QAP_CALLBACK_EVENT_DATA:
            if (data != NULL) {
                ...
                if (getmetadata && buffer && buffer->buffer_parms.output_buf_params.metadata) {
                    if (dolby_formats) {
                        audio_ms12_mix_metadata_t *metadata = (audio_ms12_mix_metadata_t *)buffer->buffer_parms.output_buf_params.metadata;
...
int main(int argc, const char *argv[])
{
    ...
    if (!session_broadcast) {
        if ((input_config->format == QAP_AUDIO_FORMAT_AAC) ||
            (input_config->format == QAP_AUDIO_FORMAT_AAC_ADTS) ||
            (input_config->format == QAP_AUDIO_FORMAT_PCM_16_BIT) ||
            (input_config->format == QAP_AUDIO_FORMAT_PCM_8_24_BIT) ||
            (input_config->format == QAP_AUDIO_FORMAT_PCM_24_BIT_PACKED) ||
            (input_config->format == QAP_AUDIO_FORMAT_PCM_32_BIT)) {
            print("Format is not supported for BD usecase");
            return -1;
        }
    }

    if (session_broadcast && has_system_input) {
    ...
    if (session_broadcast && has_assoc_input) {
    ...
    if (session_broadcast && has_main2_input) {
    ...
    if ((input_config->format == QAP_AUDIO_FORMAT_AC3) ||
        ...
        (input_config->format == QAP_AUDIO_FORMAT_AAC) ||
        (input_config->format == QAP_AUDIO_FORMAT_AAC_ADTS) ||
        (input_config->format == QAP_AUDIO_FORMAT_PCM_16_BIT) ||
        (input_config->format == QAP_AUDIO_FORMAT_PCM_8_24_BIT) ||
        (input_config->format == QAP_AUDIO_FORMAT_PCM_24_BIT_PACKED) ||
    ...
    if ((input_config->format == QAP_AUDIO_FORMAT_DTS) ||
        ...
        m8_lib_handle = qap_load_library(QAC_LIB_M8);
        dts_formats = true;
    ...
    if (!session_broadcast && dolby_formats && getmetadata) {
    ...
    while(!test_thread_exit) {
       ALOGE("wait for eos COMMAND");
       usleep(4000);
    }
