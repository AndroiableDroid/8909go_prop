    ...
    if (!p_gcs_ses->exit_buffering) {
        ...
        if (buff_sz > avail_space) {
            ...
            p_gcs_ses->rd_ptr = circ_buff_move_ptr_fwd(p_gcs_ses->circ_buff,
                rd_ptr, buff_sz - avail_space, p_gcs_ses->circ_buff_sz);
        }
    ...
    sound_trigger_sound_model_type_t sm_type __unused)
{
    ...
    if (p_hw_ses->nonpersistent_cal_size) {
        ...
        if (det_config_size) {
            ...
            if (conf_levels) {
                memcpy(msg_offset, (uint8_t *)conf_levels, num_conf_levels);
                msg_offset += num_conf_levels;
                p_msg->custom_payload_sz = num_conf_levels;
            }
        }
...
static int restart(st_hw_session_t *p_ses,
    ...
    unsigned int num_conf_levels __unused,
