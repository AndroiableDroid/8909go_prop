...
void kill_thread(pthread_t tid,bool is_join) {
	MMI_ALOGI("thread(thread id=%lu) will be killed.\n", tid);
    if(pthread_kill(tid, 0) == 0) {
		MMI_ALOGI("thread(thread id=%lu) will be killed-0.\n", tid);
        pthread_kill(tid, SIGUSR1);
		MMI_ALOGI("thread(thread id=%lu) will be killed-1.\n", tid);
		if(is_join){
			pthread_join(tid, NULL);
			MMI_ALOGI("thread(thread id=%lu) is killed-2\n", tid);
		}
    }else
    	MMI_ALOGI("thread(thread id=%lu) can not be killed\n", tid);
}
