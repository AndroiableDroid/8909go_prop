...
void switch_module(void *m) {
    ...
    MMI_ALOGI("get cur layout(%s) for module:[%s] fd is [%d]", mod->config_list[KEY_LAYOUT].c_str(), mod->module, mod->socket_fd);
...
static void do_page_up(void *m) {
    ...
    if(is_factory_tab){
    	if(reconfig("/persist/FTM_AP/mmi.cfg") <= 0){
