...
void set_boot_mode_vold(boot_mode_type mode) {

    int sock;
	char final_cmd[255] = "0 ";
  	sprintf(final_cmd, "0 setboot %d", mode);
  	ALOGI("VOLD final_cmd = %s",final_cmd);
    while ((sock = socket_local_client("vold",ANDROID_SOCKET_NAMESPACE_RESERVED,SOCK_STREAM)) < 0) {
...
void flush_result() {
    ...
    if(reset_sim_num == 1){
    	 strcpy(sim_buf,"\n[SIM_TYPE]\nResult = ssss\n");
   		 fwrite(sim_buf,sizeof(char),strlen(sim_buf),fp);	
    ...
    if(fp != NULL)
        fclose(fp);
	system("chmod 777 /persist/FTM_AP");	
	//snprintf(_path,sizeof(_path),"chmod 777 %s",g_res_file);
	system("chmod 777 /persist/FTM_AP/mmi-auto.res");
	system("chmod 777 /persist/FTM_AP/mmi.res");
}
           ...
           if(select(fd+1,&rd,NULL,NULL,NULL) < 0)
               ...
               while((nread = read(fd,buff,255))>0) {
                   ...
                   if(!strcmp(buff,"QFCT:START")){
                       char t_buf[255] = {0,};
                       int ret_cfg = reconfig("/persist/FTM_AP/mmi-auto.cfg");
						       if(ret_cfg <= 0){
				   ...
				   if(!strcmp(buff,"QFCT:END")){
                       char t_buf[255] = {0,};
                       int ret_cfg = reconfig("/persist/FTM_AP/mmi.cfg");
				       if(ret_cfg <= 0){
		                    is_factory_tab = true;
		                    nread=write(fd, "QFCT:Swtich to Factory Config Failed\r\n", strlen("QFCT:Swtich to Factory Config Failed\r\n"));
				       }else{
...
static void launch_clients() {
    ...
    for(p = g_nodup_modules_map.begin(); p != g_nodup_modules_map.end(); ++p) {
        module_info *mod = (module_info *) (p->second);
		MMI_ALOGE("mod->module name is =%s\n",mod->module);
        if(mod != NULL) {
...
static int init_config(const char *cfg, int _sim_num, int _lcd) {
	...
	char *pp = "/persist/FTM_AP/mmi-check";
    ...
    if(p != NULL) {
		...
		system("chmod 777 /persist/FTM_AP/mmi-check");
...
static void clean_resource() {
    ...
    kill_thread(g_waiting_event_tid,true);
    /**Clean Layout*/
...
int reconfig(const char *cfg) {
    ...
    if(_lay == NULL || _lay->m_listview == NULL) {
		...
		if(_module_running){
			 MMI_ALOGE("%s:Some modules are running,cannot switch to other config",__FUNCTION__);
			 return -1;
		}
    } 
...
int main(int argc, char **argv) {
	...
	while(retry<10){
		if (binder == NULL) {
			MMI_ALOGE("getService failed retry-%d",retry);
			retry++;
			sleep(1);
			binder = sm->getService(String16("service.quecservice"));
			continue;
		}
	MMI_ALOGE("getService success");
	break;
	}
    
	if(retry == 10){
		MMI_ALOGE("getService failed,use default parameters");
		w_ce_ds_ss = 2;
		_lcd_num = 1;
	}else{
		...
		MMI_ALOGE("Client Step 1: Check W CE and SIM Num is %d",w_ce_ds_ss);
		if(w_ce_ds_ss < 0){
			MMI_ALOGE("Client Parse SIM FAILED, Use default SIM Parametes");
			w_ce_ds_ss = 2;
		}
		_lcd_num = cs->_check_lcd();
		
		MMI_ALOGE("Client Parse XML\n");
		ret_parse_xml = cs->_parse_xml();
		if(ret_parse_xml <= 0){
			MMI_ALOGE("Client Parse XML ERROR\n");
		}
	}
	lcd_num = _lcd_num;
	reset_sim_num = w_ce_ds_ss;
  if(ret_parse_xml <= 0){
		MMI_ALOGE("MMI Parse XML");
		if(true == is_create_mmi_cfg() && SUCCESS != create_mmi_cfg()) {
			MMI_ALOGE("Parse mmi.xml file failed");
			return false;
		}		
		w_ce_ds_ss = 2;
		_lcd_num = 1; 
		ret = init_config("/persist/FTM_AP/mmi.cfg",w_ce_ds_ss,_lcd_num);
	}else{
	...
	/*if(ret_parse_xml <= 0){
		MMI_ALOGI("init config with default config");
		w_ce_ds_ss = 2;
		_lcd_num = 1;
		ret = init_config("/vendor/etc/mmi/mmi.cfg",w_ce_ds_ss,_lcd_num);
	}else{
		ret = init_config(MMI_CONFIG,w_ce_ds_ss,_lcd_num);
	}*/
    if(!post_config()) {
