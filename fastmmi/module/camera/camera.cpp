...
void mmi_cam_user_preview_cb(mm_camera_buf_def_t * frame) {
	MMI_ALOGE("quectel mmi camera preview cb\n");
...
int mmi_cam_set_init_params() {
    ...
    if(rc != MM_CAMERA_OK) {
		...
		for(i=0;i<10;i++){
			rc = setFocusMode(&g_mmi_cam.camera_obj, g_mmi_cam.focus_mode);
			if(rc != MM_CAMERA_OK){
				MMI_ALOGE("retry-%d set focus mode fail %s\n",i,strerror(errno));
				continue;
			}else
				break;
		...
		if(i==10){
			MMI_ALOGE("set focus mode fail after retry 10\n");
			return FAILED;
		}else{
			MMI_ALOGI("set focus mode to: %d ok\n", g_mmi_cam.focus_mode);
		}
    }
...
int mmi_cam_start_preview() {
	...
	for(retry=0;retry<5;retry++){
		ret = mm_app_start_preview(&g_mmi_cam.camera_obj);
		if(ret < 0){
			MMI_ALOGE("start camera preview ret is %d[%s]\n",ret,strerror(errno));
			sleep(1);
			continue;
		}else{
			MMI_ALOGE("start camera preview ret is ok-%d\n",ret);
			break;
		}
			
	}
	//int ret = mm_app_start_preview(&g_mmi_cam.camera_obj);
