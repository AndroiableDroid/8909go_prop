...
static int32_t module_stop(const mmi_module_t * module) {
	...
	MMI_ALOGD("stop thread(thread id=%lu) for module:[%s]", module->run_pid, module->name);

	if(is_thread_created)
		kill_thread(module->run_pid,false);
	else
		MMI_ALOGE("NULL Thread for module:[%s]", module->name);
