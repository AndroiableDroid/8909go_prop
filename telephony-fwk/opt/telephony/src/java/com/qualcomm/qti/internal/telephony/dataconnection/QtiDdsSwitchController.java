...
public class QtiDdsSwitchController {
    ...
    private boolean isCallIdle(int phoneId) {
        return (mFgCsCalls[phoneId].isIdle() &&
                ...
                mFgImsCalls[phoneId].getState() == ImsPhoneCall.State.IDLE &&
                mBgImsCalls[phoneId].getState() == ImsPhoneCall.State.IDLE &&
                mRiImsCalls[phoneId].getState() == ImsPhoneCall.State.IDLE);
    }
