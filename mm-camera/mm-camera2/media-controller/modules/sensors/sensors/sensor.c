...
int32_t sensor_probe(int32_t fd, const char *sensor_name)
{
  ...
  /* Update flash name in slave info */
  flash_name = sensor_lib_params.sensor_lib_ptr->flash_name;
  if (flash_name)
   if (strlen(flash_name) <  MAX_SENSOR_NAME)
       strlcpy(slave_info->flash_name,
                flash_name, strlen(flash_name) + 1);
