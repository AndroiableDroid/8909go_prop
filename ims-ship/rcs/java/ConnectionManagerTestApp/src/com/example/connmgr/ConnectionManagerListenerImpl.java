...
import com.qualcomm.qti.imscmservice.V1_1.IMSCM_AUTOCONFIG_REQUEST_STATUS;
...
public class ConnectionManagerListenerImpl extends IImsCmServiceListener.Stub  {
    ...
    */
    ...
    /*onConfigurationChange_1_1---- Callback invoked when the configurations are updated*/
    public void onConfigurationChange_1_1(IMSCM_CONFIG_DATA configData, int userData) {
        ...
        if(configData != null)
        {
          ...
          if (configData.deviceConfig != null)
          {
            ...
            Log.d(TAG, "deviceConfig" +
                  ...
                  ConnectionGlobalData.deviceConfigData.sPANI + "\n" +
                  ConnectionGlobalData.deviceConfigData.sUriUserPart + "\n" +
                  ConnectionGlobalData.deviceConfigData.sPLANI);
    ...
    public void onServiceReady(long serviceHandle, int userData, int status) {
        ConnectionGlobalData.connMgrInitialised = 1 ;
        ConnectionGlobalData.cmServiceHandle = serviceHandle;
        Log.d(TAG, "onServiceReady: serviceHandle :"+ ConnectionGlobalData.cmServiceHandle);
        Log.d(TAG, "onServiceReady: userData :"+ userData);
        Log.d(TAG, "onServiceReady: status :"+ status);
    }

    /*onAcsConnectionStatusChange---- Callback invoked to notify the clients about AcsRequest status.*/
    public void onAcsConnectionStatusChange(int status, int userData) {
        Log.d(TAG, "onAcsConnectionStatusChange: userData :"+ userData);
        Log.d(TAG, "onAcsConnectionStatusChange: status :"+ status);
    }

    /*OnConfigurationChange---- Callback invoked when configurations are updated*/
    public void onConfigurationChange(com.qualcomm.qti.imscmservice.V1_0.IMSCM_CONFIG_DATA configData, int userData) {
		Log.d(TAG, "onConfigurationChange_Old: Unsupported API callback");
    }
}
