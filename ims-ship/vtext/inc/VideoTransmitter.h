...
class VideoTransmitter: public VideoOmx, public VTRecorder
{
  ...
  public:
    ...
    QPE_VIDEO_ERROR ProcessRecordingFrame(uint64_t timestamp, void *dataPtr,
                                          size_t size);
    virtual QPE_VIDEO_ERROR AdaptVideoIdrPeriodicity(uint32_t iFrameInt);
  ...
  private:
    ...
    //Max QP factor used by multimedia driver and firmware to configure the encoder output port
    OMX_U32 m_maxQPFactor;
