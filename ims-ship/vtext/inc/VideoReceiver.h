...
class VideoReceiver: public VTPlayer, public VideoOmx
{
  ...
  public:
    ...
    virtual QPE_VIDEO_ERROR StartRecord(char* pFileName, int iMaxDuarion);
    virtual QPE_VIDEO_ERROR StopRecord();
    ...
    int dequeueBufferFromNativeWindow(ANativeWindow *anw, ANativeWindowBuffer** anb);
  ...
  private:
    ...
    int DropFrameCheck(long long int app_tstamp, uint8* cvo_info, int8 *retreive_index);
};
