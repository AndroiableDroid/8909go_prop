...
VTRecorder::VTRecorder()
{
  ...
  property_get("perist.vendor.ims.idr_interval", iframeInterval, "");
  property_get("perist.vendor.ims.enableslice", sliceMode, "");
  ...
  if (iframeInterval[0] != '0') {
    m_IdrFrameInt = (uint8_t)atoi(iframeInterval);
  }

  MINIMAL3("sliceMode=%d, IFrameInterval =%d", m_SliceMode, m_IdrFrameInt);
