...
void VideoCodecEncoder::videoRecordLoop()
{
  ...
  while (iRecorderState == eVideoStateActive) {
    ...
    eStatus = ProcessRecordingFrame((uint64_t)0, NULL, 0);
...
QPE_VIDEO_ERROR VideoCodecEncoder::Configure(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  do {
    ...
    if (!m_codecConfig.iLTRSupportEnabled) {
      ...
      AMediaFormat_setInt32(m_format, MEDIA_KEY_I_REFRESH_PERIOD_NPFRAMES,
                            MAX_IDR_PERIOD);
      AMediaFormat_setInt32(m_format, MEDIA_KEY_I_REFRESH_N_IDR_PERIOD, 0);
    ...
    CRITICAL5("Configure: Height %d, Width %d, FrameRate %d, IFrame interval %d",
              iHeight, iWidth,
              m_codecConfig.iFrameRate, m_codecConfig.iFrameInterval);
    //this logic is to overwrite iframe interval for testing purpose
    int iFrameInt = m_codecConfig.iFrameInterval;
    ...
    size_t ulFrameBufSize)
{
  ...
  if (eVideoStateActive == iRecorderState && mCodec) {
    /* If Encoder is configured in Byte Buffer mode, then feed data explicitly.
       This will be used in static image mode. */
    if (VIDEO_BUFFER_NON_META_MODE == m_codecConfig.iInputVideoBufferMode &&
        dataPtr) {
      ...
      if (InputBufId >= 0) {
        ...
        if (NULL == pBuf) {
          ...
          ERROR1("ProcessRecordingFrame slEncIpBufLen < ulFrameBufSize");
          return VIDEO_ERROR_UNKNOWN;
        }
        ...
        ERROR2("ProcessRecordingFrame AMediaCodec_getInputBuffer %d", InputBufId);
      }
    } //! if (VIDEO_BUFFER_META_MODE != m_codecConfig.iInputVideoBufferMode ..)
    ...
    //time out in microsecond 100ms
    auto status = AMediaCodec_dequeueOutputBuffer(mCodec, &info, 1000000);

    if (status >= 0) {
      if (info.flags & AMEDIACODEC_BUFFER_FLAG_END_OF_STREAM) {
        CRITICAL1("processrecording output EOS");
      }

      MINIMAL4("dequeueoutputbuffer buffer size %d index %d offset %d",
      ...
      if (m_bLTRFeatureOn && pFormat) {
        ...
        if (pExtraData && true == bStatus) {
          ...
          if (0xFFFF != ulDataVal) {
            CRITICAL4("Tx-LTR: ulExtraDataLen %d, ulDataVal %x, pExtraDataPtr %x",
                      ulExtraDataLen, ulDataVal, pExtraData);
      ...
      pMediaPktInfoTx->appTimeStamp =
        currentAppsTS - (getCurrentAVTime() - info.presentationTimeUs) / 1000;
      pMediaPktInfoTx->ullSysVidFrameOPts = getCurrentAVTime() / 1000;
      CRITICAL3_STR("FTB_DONE : data[4] : %x IFrame: %d", buf[4], info.flags & 0x01);

      //! Retry logic for sending I-frame till 10second
      if (m_IDRRetransmitOn && (info.flags & 0x01) && !m_LastIDRtimestamp) {
      ...
      MINIMAL2_STR("SendEncodedFramesToAppfromIOMX :: Sending frame to RTP frametime = %lu"
                   , (unsigned long)iQpFrame.sMediaPacketInfo.sMediaPktInfoTx.iTimeStamp);
      ...
      MINIMAL1("encoder output buffers released");
      AMediaCodec_releaseOutputBuffer(mCodec, status, false);
    } else if (status == AMEDIACODEC_INFO_OUTPUT_BUFFERS_CHANGED) {
      MINIMAL1("ProcessRecordingFrame: encoder output buffers changed");
      ...
      CRITICAL1("ProcessRecordingFrame: encoder no output buffer right now");
    } else {
      ERROR2("ProcessRecordingFrame: unexpected info code: %zd", status);
...
QPE_VIDEO_ERROR VideoCodecEncoder::AdaptVideoIdrPeriodicity(uint32_t iFrameInt)
{
  QPE_VIDEO_ERROR status = VIDEO_ERROR_OK;
  int iFrameRate = m_codecConfig.iFrameRate;
  CRITICAL5("AdaptVideoIdrPeriodicity for FrameRate %d  iFrameInt %d RecorderState %d, ltr %d",
            iFrameRate, iFrameInt, iRecorderState, m_bLTRFeatureOn);
