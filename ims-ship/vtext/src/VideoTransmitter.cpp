...
//Move to Encoded data function in Thinclient
  ...
  ERROR5("[THINCLIENT]engine_camera_handle_callback_timestamp : time_in_micro %llu time_in_nano %llu handle - %p size - %d",
         timestamp / 1000, timestamp, handle, handle->data[2]);
#if 0
  ...
  if (pTransmitter->getFrameRate() != 0) {
    if ((0 == previouststamp) ||
      ...
      pTransmitter->ProcessRecordingFrame((uint64)timestamp / 1000, handle,
                                          handle->data[2]);
      MINIMAL3_STR("[THINCLIENT]pushed frames timstamp difference timestamp = %lu , prevtstamp = %lu",
                   timestamp / 1000, previouststamp / 1000);
      recorder_mutex_unlock();
      return;
    } else {
      MINIMAL3_STR("[THINCLIENT]skipping frames timstamp difference timestamp = %lu , prevtstamp = %lu",
                   timestamp / 1000, previouststamp / 1000);
    }
  } else {
    MINIMAL3_STR("[THINCLIENT]no skipping frames timstamp difference timestamp = %lu , prevtstamp = %lu",
                 timestamp / 1000, previouststamp / 1000);
    pTransmitter->ProcessRecordingFrame((uint64)timestamp / 1000, handle,
                                        handle->data[2]);
  }
...
void RecordingCameraBuffer::createqueuebuffer(uint32_t height, uint32_t width)
{
  ...
  CRITICAL2("producerrecordingsurface %p", RecordingBufferProducer.get());
  CRITICAL2("consumerrecordingsurface %p", RecordingBufferConsumer.get());
  ...
  CRITICAL2("vtlib proxy %p", proxy.get());
...
//if the Queue is full
  if ((Cameratail - CameraBufferSize) == Camerahead) {
    ...
    DEBUG2("enqueue buffer %#llx to input", Buffer->buffer->getId());
    return QP_TRUE;
  }
...
void ReleaseBuffers(const sp<GraphicBuffer> &buffer, const sp<Fence> &fence)
{
  ...
  CRITICAL2_STR("releasedbuffer %#llx to input", buffer->getId());
}
...
bool CameraBufferDequeueAll(CameraBufferSlot *Buffer)
{
  if (Cameratail == Camerahead) {
    ...
    if (camerabuffers[Camerahead % CameraBufferSize].used == QP_TRUE) {
      DEBUG2("dequuebuffer %#llx to input",
             camerabuffers[Camerahead % CameraBufferSize].buffer->getId());
      ...
      DEBUG2("dequeue Cbuffer %#llx to input", Buffer->buffer->getId());
      return QP_TRUE;
    }
  }
...
void RecordingCameraBuffer::onFrameAvailable(const BufferItem& item)
{
  ...
  if ((eVideoStateActive != mTx->getRecorderState()) || (error != 0) ) {
    CRITICAL3("onFrameAvailable release buffer immediately, recordstate %d not active slot=%llu",
  ...
  if (Consumeritem.mGraphicBuffer != NULL) {
    ...
    DEBUG2("buffer handle ptr is  %p", pHandle);
    ...
    MINIMAL3("onFrameAvailable acquireBuffer CameraTS=%llu slot=%d",
             VtTimeStamp, (long long)Consumeritem.mGraphicBuffer->getId());
#endif
...
bool CameraBufferDequeue(CameraBufferSlot *Buffer, bool skip)
{
  if (Cameratail == Camerahead) {
    ...
    if (skip == camerabuffers[Camerahead % CameraBufferSize].skip) {
      ...
      Buffer->slotid = camerabuffers[Camerahead % CameraBufferSize].slotid;
      Buffer->used = camerabuffers[Camerahead % CameraBufferSize].used;
      DEBUG2("dequeue buffer %#llx to input", Buffer->buffer->getId());
      return QP_TRUE;
    } else {
      Camerahead--;
      DEBUG2("previous released buffer %#llx to input",
             camerabuffers[Camerahead % CameraBufferSize].buffer->getId());
      return QP_FALSE;
    }
  }
                             ...
                             const sp<Fence> &fence)
{
  ...
  if (CameraBufferDequeue(&camerabuffer, QP_FALSE)) {
    if (camerabuffer.used == QP_TRUE
        && camerabuffer.slotid == (long long)buffer->getId()) {
      ReleaseBuffers(camerabuffer.buffer, camerabuffer.fence);
      camerabuffer.used = QP_FALSE;
      camerabuffer.skip = QP_FALSE;
...
VideoTransmitter::VideoTransmitter()
{
  ...
  m_iMinInputBufCount = NULL;
  m_iMinOutputBufCount = NULL;
  m_iBaseTimeStamp = 0;
  m_iTimeStamp = 0;
                                       ...
                                       QP_VIDEO_CONFIG* pCodecConfig)
{
  ...
  MINIMAL3("VideoTransmitter::Init size = %d, iFrameInterval set by user %d",
           size, m_IdrFrameInt);
  //Max QP factor used by multimedia driver and firmware
  char maxQP[PROPERTY_VALUE_MAX];
  property_get("persist.ims.mm_maxqp", maxQP, "45");
  m_maxQPFactor = 45;
  m_maxQPFactor = (OMX_U32)atoi(maxQP);
  MINIMAL2("VideoTransmitter::Init: maxQPFactor set by user %d", m_maxQPFactor);
...
QPE_VIDEO_ERROR VideoTransmitter::GenerateH264IDRFrame()
{
  ...
  status_t omxresult;
  int iFrameInt = AVPF_FRAME_INTERVAL;
  ...
  do {
    ...
    CRITICAL3("VideoTransmitter::GenerateH264IDRFrame IDR=%d,Curr=%d",
              ((IDRtime.tv_sec * 1000) + (IDRtime.tv_usec / 1000)),
              ((currtime.tv_sec * 1000) + (currtime.tv_usec / 1000)));

    if (IDRtime.tv_sec == 0
        || ((((currtime.tv_sec * 1000) + (currtime.tv_usec / 1000)) - ((
               IDRtime.tv_sec * 1000) + (IDRtime.tv_usec / 1000)))) > 400) {
      IDRtime.tv_sec = currtime.tv_sec;
      IDRtime.tv_usec = currtime.tv_usec;
      /** Request an I-FRAME */
...
{
  EVENT1("In VideoTransmitter::Start::: starting the Transmitter");
  CONTROLLOG("RecorderStateTransition:Recorder:Start:%d:VIDEO_MSG_RECORDER_STARTED",
             iRecorderState, 0, 0);
  ...
  DEBUG1(" VideoTransmitter::Start::   move to Executing command ");
  CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d:OMX_CommandStateSet",
             m_IOMXState, 0, 0);

  if (m_IOMXState == OMX_StateExecuting) {
    MINIMAL1("VideoTransmitter::Start already active hence returning");
    iRecorderState = eVideoStateActive;
    CONTROLLOG("RecorderStateTransition:Recorder:VIDEO_MSG_RECORDER_STARTED:%d",
               iRecorderState, 0, 0);
  ...
  CHECK(sem_timedwait());
  CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d",
             m_IOMXState, 0, 0);
  m_IDRRetransmitOn = 1;
  ...
  for (uint i = 0; i < m_iMinOutputBufCount ; i++) {
    if (m_pEncOutpBufHeader_iomx[i] && !m_pEncOutpBufHeader_iomx[i]->usedbyIOMX ) {
      ...
      CRITICAL3_STR("Calling fill buffer on %p i = %d", m_pEncOutpBufHeader_iomx[i],
                    i);

      if (m_NodeId->fillBuffer(m_pEncOutpBufHeader_iomx[i]->buffer_id,
                               OMXBuffer::sPreset, m_pEncOutpBufHeader_iomx[i]->fencefd) != OK) {
        ERROR1(
          "VideoTransmitter::Start Failed to fill the buffer");
        return VIDEO_ERROR_UNKNOWN;
      }
    }
  }
...
{
  TRACE1("VideoTransmitter::Stop  Sending the event to stop recording");
  CONTROLLOG("RecorderStateTransition:Recorder:Stop:%d:VIDEO_MSG_RECORDER_STOPPED",
             iRecorderState, 0, 0);

  if (iRecorderState == eVideoStateStopping) {
    ERROR1("VideoTransmitter::Stop already Stopped hence returning");
    return VIDEO_ERROR_OK;
  }

  EVENT1("VideoTransmitter::Stop::   move to Pause command ");
  iRecorderState = eVideoStateStopping;
  //m_PrevTimeStamp = 0;
  //m_IDRframeinterval = 2;
  CONTROLLOG("RecorderStateTransition:Recorder:VIDEO_MSG_RECORDER_STOPPED:%d",
             iRecorderState, 0, 0);
...
/****************************************************************************************
 Start saving for videoTransmitter
 *****************************************************************************************/
QPE_VIDEO_ERROR VideoTransmitter::StartRecord(char* pFileName, int iMaxDuarion)
{
  ERROR1("VideoTransmitter::StartRecord Deprecated");
  return VIDEO_ERROR_UNKNOWN;
}

/****************************************************************************************
 Stop saving for videoTransmitter
 *****************************************************************************************/
QPE_VIDEO_ERROR VideoTransmitter::StopRecord()
{
  ERROR1("VideoTransmitter::StopRecord Deprecated");
  return VIDEO_ERROR_OK;
}
...
{
  TRACE1("VideoTransmitter::Configure");
  OMX_ERRORTYPE err = OMX_ErrorNone;
  bool isReconfiguring = QP_FALSE;
  CONTROLLOG("RecorderStateTransition:Recorder:Configure:%d:VIDEO_MSG_CODEC_CHANGED",
             iRecorderState, 0, 0);
  ...
  CONTROLLOG("Videotransmitter:Configure: eH265ProfileLevel - %d eH265ProfileType - %d",
             tCodecConfig.eH265ProfileLevel, tCodecConfig.eH265ProfileType, 0);
  CRITICAL5("configure: rotation is %d cameraheight=%d camerawidth=%d eHalVersion",
  ...
  //CRITICAL2("VideoTransmitter :: Post VIDEO_MSG_DEV_INITIALISED %x", pVideoOmx);
  CRITICAL1("VideoTransmitter :: Post VIDEO_MSG_DEV_INITIALISED");

  if (prevState == eVideoStateNotReady) {
    SendEventsToThinclient(VIDEO_MSG_DEV_INITIALISED);
    EVENT1("VideoTransmitter :: Post VIDEO_MSG_DEV_INITIALISED");
    CONTROLLOG("RecorderStateTransition:Recorder:VIDEO_MSG_DEV_INITIALISED:%d",
               iRecorderState, 0, 0);
    SendEventsToThinclient(VIDEO_MSG_CODEC_CHANGED);
  } else {
    if (m_codecConfig.iInternalCodecConfig == QP_FALSE) {
      EVENT1("VideoTransmitter :: Post VIDEO_MSG_CODEC_CHANGED");
      CONTROLLOG("RecorderStateTransition:Recorder:VIDEO_MSG_CODEC_CHANGED:%d supress_transmitter_notification - %d",
                 iRecorderState, m_bInternalEvent, 0);
      SendEventsToThinclient(VIDEO_MSG_CODEC_CHANGED);
    }

    //First transition to Exectuing state with new configuration
    EVENT1(" VideoTransmitter::Configure::   move to Executing command ");
    CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d:OMX_CommandStateSet",
               m_IOMXState, 0, 0);
...
QPE_VIDEO_ERROR VideoTransmitter::AdaptVideoFrameRate(uint32_t iFrameRate)
{
  ...
  if (res == OMX_ErrorNone) {
    idr_period.nPFrames = (OMX_U32) (iFrameRate * iFrameInt) - 1;
    CRITICAL2("VideoTransmitter:: iframeinterval.nPFrames %lu",
              idr_period.nPFrames);
    idr_period.nIDRPeriod = (OMX_U32) 1;
...
QPE_VIDEO_ERROR VideoTransmitter::AdaptVideoIdrPeriodicity(uint32_t iFrameInt)
{
  OMX_VIDEO_CONFIG_AVCINTRAPERIOD idr_period;
  ...
  iFrameRate = m_codecConfig.iFrameRate;
  ...
  int nPFrames = (int) (iFrameRate * iFrameInt) - 1;
...
{
  CRITICAL1(
    "VideoTransmitter::IOMXVideoEncEventHandler:: In encoder event handler callback");
  ...
  if (eEvent == OMX_EventCmdComplete) {
    ...
    CRITICAL2(
      " VideoTransmitter::IOMXVideoEncEventHandler::OMX error received with error type %x",
      (OMX_ERRORTYPE) Data1);
    ...
    IOMX::buffer_id buffer)
{
  ...
  if (node == NULL) {
    ERROR4(
      "TIMEDELAY::IOMXVideoEncEmptyBufferDone omx or node null, frame = %d, time = %d buff id = %p",
      tempemptybuffercallbackcountenc,
...
#endif
  CRITICAL5(
    "Profiling ENCODER->IMS EmptyBufferDone frame = %d, time = %d buffid = %p index %d",
    tempemptybuffercallbackcountenc,
    ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)), buffer, i);
  EVENT1("VideoTransmitter::IOMXVideoEncEmptyBufferDone:: exit\n");
  return;
}
                                                 ...
                                                 )
{
  ...
  gettimeofday(&tv, NULL);
  CRITICAL6_STR(
    "Profiling ENCODER->IMS Fillbufferdone frame = %d, appstime = %lu buffer = %p frametime=%llu length=%d",
    tempfillbuffercallbackcountenc,
    ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)), buffer, timestamp, range_length);
  //OMX_U8* pEncodeBuffer = (OMX_U8*) data_ptr;
  DEBUG1("VideoTransmitter::IOMXVideoEncFillBufferDone::params\n");
  TRACE4("range_offset = %d,range_length = %d,buffer = %p", range_offset,
         range_length, buffer);
  //DEBUG2(
  //"VideoTransmitter::IOMXVideoEncFillBufferDone:: pEncodeBuffer = 0x%x\n",
  //pEncodeBuffer);
  //Sending the video frames to RTP without changing the context:: TODO: need to chek after the posteventtoeventqueue redesigning
  uint8_t i = 0;
  ...
  DEBUG2(
    "VideoTransmitter::IOMXVideoEncFillBufferDone   m_pEncOutpBufHeader_iomx[i]->buffer_id = %p",
    m_pEncOutpBufHeader_iomx[i]->buffer_id);
//   timestamp = modem_time +((tv.tv_sec * 1000) + (tv.tv_usec / 1000))-apps_time;
  DEBUG5_STR(
  ...
  if (m_IDRRetransmitOn && (flags & OMX_BUFFERFLAG_SYNCFRAME)
      && !m_LastIDRtimestamp) {
    m_LastIDRtimestamp = (uint32)tv.tv_sec;;
  }

  if ((m_codecConfig.iFrameInterval > AVPF_FRAME_INTERVAL)
      && (iRecorderState == eVideoStateActive)) {
    DEBUG4_STR("m_IDRRetransmitOn %d tv.tv_sec %lu m_LastIDRtimestamp : %lu",
  ...
  timestamp = modemTS;
  CRITICAL5_STR("FTB_DONE : data[4] : %x m_IDRRetransmitOn : %d m_LastIDRtimestamp : %lu tv.tv_sec : %lu",
  ...
  for (j = 0; j < 6; j++) {
    DEBUG3("Encoder Header[%d] = %p", j, data[j]);
  }
  ...
  QP_VIDEO_CONFIG sVideoConfig)
{
  ...
  pQPRange.minQP = 1;
  pQPRange.maxQP = m_maxQPFactor;
  CRITICAL2("Output Port minQP: %d\n", pQPRange.minQP);
  CRITICAL2("Output Port maxQP: %d\n", pQPRange.maxQP);
    ...
    if (countencoder < EncoderBufferSize) {
      ...
      if (status == NO_ERROR) {
        MINIMAL5_STR("IMS->Encoder queuebuffer CameraTS= %llu localid=%llx,cameraid=%llx currentavtime=%llu",
                     ...
                     (long)(long)Consumeritem->mGraphicBuffer->getId(), currentAVTime);
      }
    }
    ...
    for (; i < m_iMinInputBufCount; i++) {
      ...
      */
      ...
      if (m_pEncInpBufHeader_iomx[i]->filllen == 0) { //reuse the buffer which is free
        DEBUG3(
          "VideoTransmitter::ProcessImageFrame alloclen: %d m_pEncInpBufHeader_iomx[i]->pBuffer = %d\n",
        ...
        if (m_pEncInpBufHeader_iomx[i]->pBuffer) {
          ...
          DEBUG3("VideoTransmitter::Copying to destbuffer of size - %d, src size %d",
                 m_pEncInpBufHeader_iomx[i]->alloclen, size);

          if ( srcbuffer && destbuffer ) {
            memcpy(destbuffer, srcbuffer, size);
          } else {
            ERROR5("VideoTransmitter::destbuffer - %p srcbuffer - %p dstSize %d srcSize ",
                   destbuffer, srcbuffer, m_pEncInpBufHeader_iomx[i]->alloclen, size);
            return VIDEO_ERROR_OK;
          }
          ...
          currentAppsTS = ((tv.tv_sec * 1000) + (tv.tv_usec / 1000));
          EVENT3(
            "VideoTransmitter::ProcessImageFrame:: Before emptying the buffer with bufflen = %d and pointer = %x",
            m_pEncInpBufHeader_iomx[i]->alloclen,
            m_pEncInpBufHeader_iomx[i]->pBuffer);
          CRITICAL6_STR("Profiling IMS-> ENCODER EmptyBuffer = %d,localtime= %lu buffid = %pindex = %d,len=%d",
                        tempemptybuffercallbackcountenc + 1, currentAppsTS,
                        m_pEncInpBufHeader_iomx[i]->buffer_id, i, m_pEncInpBufHeader_iomx[i]->alloclen);
          ...
          CRITICAL4_STR("Profiling IMS->ENCODER ETB ts : %llu appstime = %lu currentAVTime %llu",
                        timestamp, ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)), currentAVTime);
...
// To be ported for HAL1 the API here is wrong
          ...
          EVENT3(
            "VideoTransmitter::ProcessImageFrame :: input buffer is NULL and can not empty the buffer %d	and %x\n",
            m_pEncInpBufHeader_iomx[i]->alloclen,
            m_pEncInpBufHeader_iomx[i]->pBuffer);
        }
      }
  ...
  if (eVideoStateActive == iRecorderState) {
    ...
    if (m_codecConfig.eHALVersion == CAMERA_HAL3) {
      ...
      if (native_hdl != NULL) {
        ...
        CRITICAL6_STR("ProcessrecordingFrame handle-> %p fd - %d,size - %d usage=%p format=%p",
      ...
      if (pbuffer == MAP_FAILED) {
        ERROR4("Failed to mmap pmem with fd = %d, size = %d errno=%d", fd, size,
               errnum);
      } else {
      ...
      for (; i < m_iMinInputBufCount; i++) {
        ...
        */
        ...
        if (m_pEncInpBufHeader_iomx[i]->filllen == 0) { //reuse the buffer which is free
          DEBUG3(
            "VideoTransmitter::ProcessRecordingFrame alloclen: %d m_pEncInpBufHeader_iomx[i]->pBuffer = %d\n",
            m_pEncInpBufHeader_iomx[i]->alloclen,
            m_pEncInpBufHeader_iomx[i]->pBuffer);
          ...
          if (m_pEncInpBufHeader_iomx[i]->pBuffer) {
            ...
            if (m_codecConfig.iInputVideoBufferMode == VIDEO_BUFFER_META_MODE) {
              ...
              CRITICAL2("VideoTransmitter::Copying to destbuffer of size - %d", size);

              if ( srcbuffer && destbuffer ) {
                memcpy(destbuffer, srcbuffer, size);
              } else {
                CRITICAL3("VideoTransmitter::destbuffer - %p srcbuffer - %p ", destbuffer,
                          srcbuffer);
                return VIDEO_ERROR_OK;
              }
            } else {
            ...
            currentAppsTS = ((tv.tv_sec * 1000) + (tv.tv_usec / 1000));
            EVENT3(
              "VideoTransmitter::ProcessRecordingFrame:: Before emptying the buffer with bufflen = %d and pointer = %x",
              m_pEncInpBufHeader_iomx[i]->alloclen,
              m_pEncInpBufHeader_iomx[i]->pBuffer);
            CRITICAL6_STR("Profiling IMS-> ENCODER EmptyBuffer = %d,localtime= %lu buffid = %pindex = %d,len=%d",
                          tempemptybuffercallbackcountenc + 1, currentAppsTS,
...
#endif
            DEBUG1(
              "Return from VideoTransmitter::ProcessRecordingFrame\n");
            return VIDEO_ERROR_OK;
          } else {
            EVENT3(
              "VideoTransmitter::ProcessRecordingFrame :: input buffer is NULL and can not empty the buffer %d  and %x\n",
              m_pEncInpBufHeader_iomx[i]->alloclen,
              m_pEncInpBufHeader_iomx[i]->pBuffer);
          }
        }
...
//}
  ...
  TRACE1(
    "VideoTransmitter::ConfigureDecoder::acquired first lock moving to OMX_StateIdle");
  CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d:OMX_CommandStateSet",
             m_IOMXState, 0, 0);
  ...
  if (!isTrebleEnabled) {
    if (tCodecConfig.iInternalCodecConfig == QP_FALSE) {
      ...
      m_mDealer[InputportFmt.nPortIndex] = new MemoryDealer(totalSize,
          "VideoTransmitter");
      CRITICAL2(
        "VideoTransmitter::Configure::allocating input Memory Dealer totalsize - %d\n",
        totalSize);
    } else {
      CRITICAL2(
        "VideoTransmitter::Configure::already input allocated Memory Dealer totalsize - %d\n",
        totalSize);
    }
...
//    if (videoInjection[0] == '1') {
  ...
  CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d",
             m_IOMXState, 0, 0);
  CRITICAL1("VideoTransmitter::Configure::Encoder Configure done");
  return VIDEO_ERROR_OK;
}
  ...
  OMX_U32 iPort, QP_VIDEO_CONFIG sVideoConfig)
{
  ...
  iFrameInt = sVideoConfig.iFrameInterval;

  if (m_IdrFrameInt) {
    iFrameInt = m_IdrFrameInt;
  }

  CRITICAL2("iFrameInterval value is %d", iFrameInt);
  ...
  h264.nPortIndex = DIR_OUT;
  h264.nPFrames = (iFrameInt * sVideoConfig.iFrameRate) -
                  1; // * iFrameInt - 1;//sVideoConfig.iFrameRate;

  if (m_bLTRFeatureOn) {
    h264.nPFrames = MAX_IDR_PERIOD;
  }
  ...
  if (result == OMX_ErrorNone) {
    //! If LTR Featur is used, then frequent IDR frames are not needed
    //! Frequent LTR frames are requested  through other config param
    if (m_bLTRFeatureOn) {
      idr_period.nIDRPeriod = (OMX_U32) 0;
      idr_period.nPFrames = MAX_IDR_PERIOD;
    } else {
      idr_period.nPFrames = (OMX_U32) (sVideoConfig.iFrameRate * iFrameInt) - 1;
      idr_period.nIDRPeriod = (OMX_U32) 1;
    }

    CRITICAL2("Tx-LTR:: iframeinterval.nPFrames %lu", idr_period.nPFrames);
    ...
    QP_VIDEO_CONFIG sVideoConfig)
{
  ...
  if (sVideoConfig.iFrameInterval) {
    iFrameInt = sVideoConfig.iFrameInterval;
  }
...
{
  TRACE2("VideoTransmitter::SendEncodedFramesToAppfromIOMX %llu", timestamp);
  ...
  if (pVideoBufferInfo->pBuffer  && range_length > 0) {
    ...
    DEBUG3(
      "VideoTransmitter::SendEncodedFramesToAppfromIOMX:: timestamp before sending it to RTP %d and base time stamp is %d",
      iTimeStamp, m_iBaseTimeStamp);
    ...
    CRITICAL3_STR("VideoTx::SendEncodedFramesToAppfromIOMX :: Sending frame to RTP with frame number %d frametime = %llu",
                  m_ulFrameCount, pMediaPktInfoTx->iTimeStamp);
...
{
  status_t err;
  CONTROLLOG("RecorderStateTransition:Recorder:TearDown:%d:VIDEO_MSG_DEV_UNINITIALISED",
             iRecorderState, 0, 0);
  //QpDplImsThinClient *ptr = QpDplImsThinClient::getInstance();
  //ptr->setVideoTransmitterParams((void *) NULL, (void *) NULL);
  EVENT3("VideoTransmitter::TearDown in recorder state = %d omx state = %d",
         iRecorderState, m_IOMXState);
  CONTROLLOG("RecorderStateTransition:OMX_Encoder:OMX_CommandStateSet:%d:OMX_CommandStateSet",
             m_IOMXState, 0, 0);
  ...
  if (m_IOMXState == OMX_StateIdle) {
    ...
    if (sem_timedwait() != VIDEO_ERROR_OK) {
      ...
      CONTROLLOG("RecorderStateTransition:Recorder:VIDEO_MSG_DEV_UNINITIALISED:%d",
                 iRecorderState, 0, 0);
