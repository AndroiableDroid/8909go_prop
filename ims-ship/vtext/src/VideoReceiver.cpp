...
int VideoReceiver::DropFrameCheck(long long int app_tstamp, uint8* cvo_info,
                                  int8 *retreive_index)
{
  for (int i = 0; i < FRAME_STATE_SET_SIZE; i++) {
    EVENT5(
      "DropFrameCheck belongs_to_drop_set %d  frameStateIndex %d source_time_video_frame %llu, app_tstamp_in_us %llu",
      frameStateSet[i].belongs_to_drop_set, i,
      frameStateSet[i].source_time_video_frame,
      frameStateSet[i].app_tstamp_in_us);
                                      ...
                                      int belongs_to_drop_set, long long int app_tstamp)
{
  ...
  EVENT6("AddToFrameStateMap dropset %d seqno %d frameStateIndex %d source_time_video_frame %u, app_tstamp_in_us %llu",
...
void VideoReceiver::ClearScreen()
{
  ...
  ERROR2("VideoReceiver::ClearScreen::HandleEvent inside for loop with err value %d %d\n",
         err1);
  ...
  if (err1 == 0) {
    ...
    for (; i < m_iMinOutputBufCount; i++) {
      if (m_pDecOutpBufHeader_iomx[i]
        ...
        //{
        DEBUG4(
          "m_pDecOutpBufHeader_iomx handle comparision %x %x %d",
...
void VideoReceiver::FillNativeBuffer()
{
  ...
  err1 =  dequeueBufferFromNativeWindow(mNativeWindow.get(), &buf);
  EVENT2("VideoReceiver::HandleEvent inside for loop with err value %d %d\n",
         err1);
  ...
  if (err1 != 0) {
    ...
    for (; i < m_iMinOutputBufCount; i++) {
      if (m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer != NULL) {
        DEBUG4("m_pDecOutpBufHeader_iomx handle comparision %x %x %d",
               m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer->handle,
               buf->handle, i);
    ...
    CRITICAL3_STR("VideoReceiver::HandleEvent dequeue buffer calling fillBuffer on buffer %p index %d",
                  m_pDecOutpBufHeader_iomx[i]->buffer_id, i);
    m_pDecOutpBufHeader_iomx[i]->usedbyIOMX = QP_TRUE;
    DEBUG4("FillNativeBuffer :: FILL BUFFER index %d buffer = %p fencefd : %d", i,
           m_pDecOutpBufHeader_iomx[i]->buffer_id, m_pDecOutpBufHeader_iomx[i]->fencefd);
...
void VideoReceiver::SetNativeWindow(sp<ANativeWindow> surfaceFar)
{
  ...
  CRITICAL4("mNativeWindowSurfaceFar = %p %d", mNativeWindowSurfaceFar.get(),
            __LINE__, 0);
  surfaceChanged = true;
...
QPE_VIDEO_ERROR VideoReceiver::Start()
{
  ...
  if (m_IOMXState == OMX_StateExecuting) {
    ...
    CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_PLAYER_STARTED:%d",
               iPlayerState, 0, 0);
    SendEventsToThinclient(VIDEO_MSG_PLAYER_STARTED);
  ...
  CONTROLLOG("PlayerStateTransition:OMX_Decoder:OMX_CommandStateSet:%d",
             m_IOMXState, 0, 0);

  if (m_IOMXState != OMX_StateExecuting) {
    CRITICAL1("Component in Executing state and breaking from the loop");
    return VIDEO_ERROR_UNKNOWN;
  }
  ...
  if (prevState == eVideoStateIdle) {
    ...
    for (uint8_t i = 0; i < portdef.nBufferCountMin; i++) {
      ...
      gettimeofday(&tv, NULL);
      CRITICAL3(
        "TIMEDELAY::VideoReceiver::Start call frame = %d, time = %d ",
        tempfillbuffercallbackcount + 1,
        ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));

      if (m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer != NULL) {
        m_pDecOutpBufHeader_iomx[i]->usedbyIOMX = QP_TRUE;
        CRITICAL4("Start:: FILL BUFFER index %d buffer = %pfencefd : %d", i,
                  m_pDecOutpBufHeader_iomx[i]->buffer_id, m_pDecOutpBufHeader_iomx[i]->fencefd);
        status_t err = m_NodeId->fillBuffer(m_pDecOutpBufHeader_iomx[i]->buffer_id,
                                            OMXBuffer::sPreset,
...
void VideoReceiver::RenderFrame(void* data)
{
  ...
  if (iPlayerState == eVideoStateActive && !m_codecConfig.iErrorState) {
    timeval tv;
    gettimeofday(&tv, NULL);
    EVENT4("TIMEDELAY::VideoDecFillBuffer call frame queuebuffer = %d, time = %d, render frame = %d ",
           tempfillbuffercallbackcount + 1,
           ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)), m_RenderFrameCount);
...
QPE_VIDEO_ERROR VideoReceiver::Stop()
{
  ...
  CONTROLLOG("PlayerStateTransition:OMX_Decoder:OMX_CommandStateSet:%d:OMX_CommandStateSet",
             m_IOMXState, 0, 0);
#if 0
...
//    m_ePlayerState = VIDEO_CODEC_PENDING; // removing this state transition as we can get player start without a codec configure request
    /*If surface is set to NULL, intermittently then this flag will help to call
      start() API*/
    m_bPlayerStartReq = true;
    CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_PLAYER_STOPPED:%d",
               iPlayerState, 0, 0);
    //ClearScreen();
    SendEventsToThinclient(VIDEO_MSG_PLAYER_STOPPED);
  }
...
/****************************************************************************************
 Start saving for videoReceiver
 *****************************************************************************************/
QPE_VIDEO_ERROR VideoReceiver::StartRecord(char* pFileName, int iMaxDuarion)
{
  ERROR1("VideoReceiver::StartRecord Deprecated");
  return VIDEO_ERROR_OK;
}
/****************************************************************************************
 Stop saving for videoReceiver
 *****************************************************************************************/
QPE_VIDEO_ERROR VideoReceiver::StopRecord()
{
  ERROR1("VideoReceiver::StopRecord Deprecated");
  return VIDEO_ERROR_OK;
}
...
int VideoReceiver::VerifyFillBuffersDone()
{
  ...
  for (i = 0; i < m_iMinOutputBufCount; i++) {
    if (m_pDecOutpBufHeader_iomx[i]
        && m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer != NULL
        && m_pDecOutpBufHeader_iomx[i]->usedbyIOMX == QP_TRUE) {
      CRITICAL3("VideoReceiver::VerifyFillBuffersDone  returning QP_FALSE used by %d buffer_id = %p",
...
QPE_VIDEO_ERROR VideoReceiver::Configure(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  CONTROLLOG("PlayerStateTransition:Player:CodecSet:%d:VIDEO_MSG_CODEC_CHANGED",
             iPlayerState, 0, 0);
  ...
  if (iPlayerState == eVideoStateIdle) {
    CONTROLLOG("VideoReceiver::Configure MovetoLoadedState current state : %d",
               iPlayerState, 0, 0);
    MovetoLoadedState();
    CONTROLLOG("VideoReceiver::Configure ReConfigureDecoder current state : %d",
               iPlayerState, 0, 0);
    ...
    CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_CODEC_CHANGED:%d",
               iPlayerState, 0, 0);
    SendEventsToThinclient(VIDEO_MSG_CODEC_CHANGED);
    return VIDEO_ERROR_OK;
  }
  ...
  if (iPlayerState == eVideoStateActive) {
    ...
    EVENT1("VideoReceiver :: Post VIDEO_MSG_CODEC_CHANGED");
    CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_CODEC_CHANGED:%d",
               iPlayerState, 0, 0);
    SendEventsToThinclient(VIDEO_MSG_CODEC_CHANGED);
    return VIDEO_ERROR_OK;
...
//      CRITICAL2("aFTER Alloctae node Component :VIDEO_CODEC_HEVC -%d \n", m_NodeId);
  ...
  m_ePlayerState = VIDEO_CODEC_CONFIGURED;
  EVENT1("VideoReceiver :: Post VIDEO_MSG_CODEC_CHANGED");
  CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_CODEC_CHANGED:%d, m_ePlayerState %d",
             iPlayerState, m_ePlayerState, 0);
  SendEventsToThinclient(VIDEO_MSG_CODEC_CHANGED);
  EVENT2("VideoReceiver::Configure fullyDone state = %d", iPlayerState);
    ...
    OMX_EVENTTYPE eEvent, OMX_U32 Data1, OMX_U32 Data2)
{
  ...
  if (eEvent == OMX_EventCmdComplete) {
    ...
    if (iCommandReceived == OMX_CommandPortEnable) {
      ...
      sem_post(&port_settings_semaphore);
      CONTROLLOG("PlayerStateTransition:OMX_Decoder:OMX_CommandPortEnable:%d",
                 m_IOMXState, 0, 0);
    ...
    OMX_STATETYPE iStateReceived = (OMX_STATETYPE) Data2;
    CONTROLLOG("VideoReceiver::IOMXVideoDecEventHandler iStateReceived : %x",
               iStateReceived, 0, 0);
    ...
    CONTROLLOG("VideoReceiver::IOMXVideoDecEventHandler::OMX error received with error type %x\n",
               (OMX_ERRORTYPE) Data1, 0, 0);
...
void VideoReceiver::ReleaseMediaBuffersAfterReconfigure()
{
  ...
  for (uint8_t i = 0; i < m_iMinOutputBufCount; i++) {
    if (m_pDecOutpBufHeader_iomx[i]) {
      ...
      if (m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer != NULL) {
        EVENT4(
          "VideoReceiver::ReConfigurePort line no %d setobserver for %x to null index %d",
          __LINE__, m_pDecOutpBufHeader_iomx[i]->mGraphicBuffer.get(), i);
  ...
  gettimeofday(&tv, NULL);
  CRITICAL2("TIMEDELAY::eVideoStateReConfiguring end time = %d",
            ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));
    ...
    IOMX::buffer_id buffer)
{
  ...
  timeval tv;
  gettimeofday(&tv, NULL);
  CRITICAL4_STR("Profiling Decoder->IMS EmptyBufferDone frame = %d, appstime = %lu buffer=%p",
                tempemptybuffercallbackcount,
  ...
  DEBUG2("VideoReceiver :: IOMXVideoDecEmptyBufferDone id  = %p", buffer);
  return;
}
                                              ...
                                              )
{
  ...
  timeval tv;
  gettimeofday(&tv, NULL);
  CRITICAL5_STR("Profiling Decoder->IMS FillBufferDone frame = %d, appstime = %lu buffer=%p FBD Time - %lu",
                tempfillbuffercallbackcount,
                ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)), buffer, timestamp);
  //CRITICAL2("DEBUGGING OMX ISSUE FBD time %lu",timestamp);
  DEBUG3("VideoReceiver::IOMXVideoDecFillBufferDone:: Received the decoded frame with frame len= %d and player state is %d\n",
  ...
  EVENT2("range_length = %d\n", range_length);
  char *data1 = (char *) m_pDecOutpBufHeader_iomx[i]->pBuffer;
  buffer_id = i;
  DEBUG2("VideoReceiver::IOMXVideoDecFillBufferDone:: buffer id : %d",
         buffer_id);
  DEBUG2("VideoReceiver::IOMXVideoDecFillBufferDone:: buffer : %p", buffer);
  DEBUG2("VideoReceiver :: IOMXVideoDecFillBufferDone id  = %p", buffer);
  DEBUG2(" IOMXVideoDecFillBufferDone m_iMinOutputBufCount : %d",
    ...
    for (; i < m_iMinInputBufCount; i++) {
      if (m_pDecInpBufHeader_iomx[i]) {
        if (m_pDecInpBufHeader_iomx[i]->filllen == 0) {
          timeval tv;
          gettimeofday(&tv, NULL);
          EVENT2("TIMEDELAY::emptyBuffer start time = %d",
                 ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));
          memcpy(m_pDecInpBufHeader_iomx[i]->pBuffer,
                 pFrame->pData, pFrame->iDataLen);
          m_pDecInpBufHeader_iomx[i]->filllen = pFrame->iDataLen;
          EVENT2(
            "VideoReceiver::VideoPlayFrameEx:; emptying the buffer with buffer len = %d\n",
            m_pDecInpBufHeader_iomx[i]->filllen);
          gettimeofday(&tv, NULL);
          CRITICAL5(
            "IMS->Decoder EmpytBuffer call frame = %d, appstime = %lu emptyBuffer = %p len=%d\n",
          ...
          gettimeofday(&tv, NULL);
          EVENT2("TIMEDELAY::emptyBuffer end time = %d",
                 ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));
          return VIDEO_ERROR_OK;
        }
      }
...
//  OMX_PARAM_PORTDEFINITIONTYPE portInpFmt;
  ...
  if (!isTrebleEnabled) {
    ...
    for (uint8_t i = 0; i < portInpFmt.nBufferCountActual; i++) {
      ...
      if (hidlMemory != NULL) {
        ...
        DEBUG3("m_pDecInpBufHeader_iomx[%d]->pBuffer %p\n", i,
               m_pDecInpBufHeader_iomx[i]->pBuffer);
      }
    }
  }
  ...
  CRITICAL3("Configure Height : %lu width: %lu",
...
#endif
  CRITICAL3("VideoReceiver::ConfigureDecoder native_window_set_buffers_geometry width: %lu hegith: %lu",
  ...
  EVENT2("VideoReceiver::native_window_set_usage usage=0x%lx", usage);
  EVENT1(
  ...
  if (err != OK) {
    ERROR3(
      "VideoReceiver::failed while setting nBufferCountActual to %lu error no = %d",
      newBufferCount, err);
    return VIDEO_ERROR_UNKNOWN;
  }
  ...
  CRITICAL3("NATIVE_WINDOW_SET_BUFFERS_SIZE : %d err : %d", portdef.nBufferSize,
            err);
  CRITICAL4("VideoReceiver:: allocated buffer count %lu buffer size = %lu minimum buffer count = %lu",
  ...
  for (; index < portdef.nBufferCountActual; index++) {
    ...
    EVENT3("registered graphic buffer with ID %p (pointer = %p)", bufferId,
...
QPE_VIDEO_ERROR VideoReceiver::CropPort()
{
  ...
  EVENT2("Resolution information in VideoReceiver::CropPort is %p", &info);
  ...
  if ((iCodecChanged == QP_TRUE) && (m_codecConfig.eCodec == VIDEO_CODEC_H264)) {
    CRITICAL1("VideoReceiver::Cropport:: feeding SPS/PPS %d");
    iCodecChanged = QP_FALSE;
    VideoPlayVOLFrame();
  }
...
{
  status_t omxresult;
  CONTROLLOG("PlayerStateTransition:Player:ReConfigurePort:%d:VIDEO_INTERNAL_EVENT_STATUS_MSG",
             iPlayerState, 0, 0);
  ...
  if (iPlayerState == eVideoStateReConfiguring) {
    timeval tv;
    gettimeofday(&tv, NULL);
    EVENT2("TIMEDELAY::eVideoStateReConfiguring start time = %d",
           ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));
    CONTROLLOG("PlayerStateTransition:OMX_Decoder:OMX_CommandPortDisable:%d:OMX_CommandPortDisable",
               m_IOMXState, 0, 0);
    reconfiguring = true;
    Releasing_state = 1;
...
QPE_VIDEO_ERROR VideoReceiver::EnablePort(uint8_t iPort)
{
  ...
  status_t omxresult;
  CONTROLLOG("PlayerStateTransition:Player:EnablePort:%d:VIDEO_INTERNAL_EVENT_STATUS_MSG",
             iPlayerState, 0, 0);
  ...
  CRITICAL3("VideoReceiver::EnablePort:: m_iMinInputBufCount = %d Actual = %lu",
            m_iMinInputBufCount, InpportFmt.nBufferCountActual);
  CONTROLLOG("PlayerStateTransition:OMX_Decoder:OMX_CommandPortEnable:%d:OMX_CommandPortEnable",
             m_IOMXState, 0, 0);
  ...
  EVENT2("VideoReceiver:: EnablePort window=%p\n", mNativeWindow.get());
  CRITICAL3("VideoReceiver:: EnablePort  Reconfigure Height : %lu width: %lu",
  ...
  CRITICAL3("Reconfigure portdef slice height: %lu stride: %lu",
...
#endif
  CRITICAL3("VideoReceiver:: EnablePort  native_window_set_buffers_geometry width: %lu hegith: %lu",
            portdef.format.video.nFrameWidth,
            portdef.format.video.nFrameHeight);
#if 0
  PlayerMsg info;
  ...
  EVENT2("VideoReceiver:: EnablePort native_window_set_usage usage=0x%lx",
         usage);
  ...
  if (err != OK) {
    ERROR3("VideoReceiver::EnablePort:failed while setting nBufferCountActual to %lu error no = %x",
           newBufferCount, err);
    m_iMinOutputBufCount = 0;
    return VIDEO_ERROR_UNKNOWN;
  }
  ...
  for (; index < portdef.nBufferCountActual; index++) {
    ...
    m_pDecOutpBufHeader_iomx[index]->usedbyIOMX = QP_FALSE;
    m_pDecOutpBufHeader_iomx[index]->fencefd = -1;
    EVENT3("VideoReceiver::registered graphic buffer with ID %p (pointer = %p)",
  ...
  for (i = 0; i < portFmt.nBufferCountMin; i++) {
    ...
    timeval tv;
    gettimeofday(&tv, NULL);
    CRITICAL4_STR("Profiling IMS->Decoder EnablePort FillBuffer call frame = %d, appstime = %lu buffer=%p ",
                  tempfillbuffercallbackcount + 1,
                  ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)),
                  m_pDecOutpBufHeader_iomx[i]->buffer_id);
    EVENT2("VideoReceiver ::EnablePort fillBuffer = %p",
           m_pDecOutpBufHeader_iomx[i]->buffer_id);
  ...
  if (m_codecConfig.iCropSet == QP_TRUE) {
    CropPort();
  } else {
    //Transition to Active state only if we were in Reconfiguring state earlier
    //There is a possibility we could have got a Port Reconfigure before we do this
    if (iPlayerState == eVideoStateReConfiguring) {
      iPlayerState = eVideoStateActive;
    }

    /* Posting the vt_playframe semaphore */
    CRITICAL1("Videoreceiver::Enableport:: [vt_playframe]sem_post");
    sem_post_videoplay();
  }

  if ((iCodecChanged == QP_TRUE) && (m_codecConfig.eCodec == VIDEO_CODEC_H264)) {
    CRITICAL1("VideoReceiver::EnablePort:: feeding SPS/PPS %d");
    iCodecChanged = QP_FALSE;
    VideoPlayVOLFrame();
  }
...
QPE_VIDEO_ERROR VideoReceiver::TearDown()
{
  ...
  if (m_IOMXState == OMX_StatePause || m_IOMXState == OMX_StateExecuting) {
    ...
    if (sem_timedwait() != VIDEO_ERROR_OK) {
      ...
      CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_DEV_UNINITIALISED:%d",
                 iPlayerState, 0, 0);
  ...
  CONTROLLOG("PlayerStateTransition:Player:VIDEO_MSG_DEV_UNINITIALISED:%d",
             iPlayerState, 0, 0);
  SendEventsToThinclient(VIDEO_MSG_DEV_UNINITIALISED);
  EVENT3("VideoReceiver::TearDown - COMPLETE state = %d player state = %d",
         m_IOMXState, iPlayerState);
  EVENT1("VideoReceiver::TearDown - free node");
  ...
  if (iVolLen > 0 && iVolLen < VIDEOOMX_MAX_VOL_LENGTH  && pVolFrame != NULL) {
    for (uint8_t i = 0; i < m_iMinInputBufCount; i++) {
      if (m_pDecInpBufHeader_iomx[i]->filllen == 0) {
        ...
        DEBUG2(
          "VideoReceiver::VideoPlayVOLFrame:; emptying the buffer with buffer len = %d",
          m_pDecInpBufHeader_iomx[i]->filllen);
        timeval tv;
        gettimeofday(&tv, NULL);
        CRITICAL3(
          "TIMEDELAY::IOMXVideoDecEmptyBuffer call frame = %d, time = %d",
          tempemptybuffercallbackcount + 1,
          ((tv.tv_sec * 1000) + (tv.tv_usec / 1000)));
